...
module BlockApps.Bloc22.Database.Queries.Deprecated (
  getContractXabiFromMetaDataIdDeprecated
  )where
...
-}
getXabiFunctionsQuery :: (HasCallStack, MonadIO m, MonadLogger m, HasBlocSQL m) =>
                         Int32 -> m (Map Text Func)
...
WHERE XF.is_constructor = false AND XF.contract_metadata_id = $1;
-}
getXabiConstrQuery :: (HasCallStack, MonadIO m, MonadLogger m, HasBlocSQL m) =>
                      Int32 -> m (Maybe Func)
...
-}
getXabiFunctionsArgsQuery :: (MonadIO m, MonadLogger m, HasBlocSQL m) =>
                             Int32 -> m (Map Text Xabi.IndexedType)
...
-}
getXabiFunctionsReturnValuesQuery :: (HasCallStack, MonadIO m, MonadLogger m, HasBlocSQL m) =>
                                     Int32 -> m [Xabi.IndexedType]
...
-}
getXabiVariablesQuery :: (MonadIO m, MonadLogger m, HasBlocSQL m) =>
                         Int32 -> m (Map Text Xabi.VarType)
...
getXabiType :: (HasCallStack, MonadIO m, MonadLogger m, HasBlocSQL m) =>
               Int32 -> m Xabi.Type
...
getXabiStructFields :: (MonadIO m, MonadLogger m, HasBlocSQL m) =>
                       Int32 -> m [(Text, Xabi.FieldType)]
...
getXabiEnumNames :: (MonadIO m, MonadLogger m, HasBlocSQL m) =>
                    Int32 -> m [Text]
...
getXabiTypeDefs :: (MonadIO m, MonadLogger m, HasBlocSQL m) =>
                   Int32 -> m (Map Text Xabi.Def.Def)
...
{-
getContractXabiDeprecated :: (MonadIO m, MonadLogger m, HasBlocSQL m) =>
                             ContractName -> Account -> m Xabi
...
-}
getContractXabiFromMetaDataIdDeprecated :: (MonadIO m, MonadLogger m, HasBlocSQL m) =>
                                           Int32 -> m Xabi
