...
spec = do
    ...
    describe "stringToIAddr" $ do
        it "converts IPv4 addresses" $ do
            stringToIAddr "192.168.1.17" `shouldBe`
                (IPV4Addr $ 192 + (168 `shift` 8) + (1 `shift` 16) + (17 `shift` 24))

        it "converts IPv6 addresses" $ do
            stringToIAddr "2001:0db8:85a3:0000:0000:8a2e:0370:7334" `shouldBe`
                (IPV6Addr (536939960,2242052096,35374,57701172))
