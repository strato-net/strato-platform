...
data Context =
    Context {
        ...
        vmTrace             :: [String],
        unseqSink           :: forall m . (MonadIO m, K.HasKafkaState m) => ConduitT [IngestEvent] Void m (),
        vmEventsSink        :: forall m . (MonadIO m, K.HasKafkaState m, HasSQLDB m) => ConduitT [VMEvent] Void m (),
