...
postFuzz :: ToJSON a
         => (SourceMap -> T.Text -> T.Text -> T.Text -> T.Text -> IO (Either [SourceAnnotation T.Text] a))
         -> FuzzerArgs
         -> Handler A.Value
postFuzz fuzz (FuzzerArgs a b c d e) = either A.toJSON A.toJSON <$>
 liftIO (fuzz a b c d e)
