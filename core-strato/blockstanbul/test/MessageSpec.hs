...
spec = parallel $ do
  describe "RLP - quorum test vectors" $ do
    let vw = View 54975581388 45212608023800330
        ...
        sig = ExtendedSignature (HK.Signature (fromIntegral $ bytesToWord256 [0..31]) (fromIntegral $ bytesToWord256 [32..63])) True
        seal = ExtendedSignature (HK.Signature (fromIntegral $ bytesToWord256 [64..95]) (fromIntegral $ bytesToWord256 [96..127])) False
    it "matches on serializing Preprepares" $ do
      let blk = Block {
          ...
          msg = Preprepare (MsgAuth addr sig) vw blk
    ...
    it "matches on serializing Prepares" $ do
      let msg = Prepare (MsgAuth addr sig) vw digest
    ...
    it "matches on serializing Commits" $ do
      let msg = Commit (MsgAuth addr sig) vw digest seal
    ...
    it "matches on serializing RoundChanges" $ do
      let msg = RoundChange (MsgAuth addr sig) vw
