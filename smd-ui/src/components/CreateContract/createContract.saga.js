...
export function createContractApiCall(contract, src, username, address, password, args, options) {
  return fetch(url.replace(":user", username).replace(":address", address), {
    ...
    body: JSON.stringify({contract, value: 0, password, src, args, options})
...
export function * createContract(action) {
  try {
    let response = yield call(createContractApiCall, action.payload.contract, action.payload.fileText, action.payload.username, action.payload.address, action.payload.password, action.payload.arguments, action.payload.options);
