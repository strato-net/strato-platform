...
class DevDash extends Component {
    ...
    render() {
      ...
      function cardProducer(name, redirect, description, image, key) { return (
      <div className="row col-md-6" key={key}>
      ...
      const cards = appsList.map((obj, index) => {
        var key = index;
        return cardProducer(obj['appName'],  obj['urlToApp'], obj['description'], obj['image'], key);
      });
