...
contract Describe_ArithmeticMinimal_LocalCopy {
    ...
    function it_infinite_loop_reverts() public {
        ...
        require(ok, "infinite loop did not revert");
    }

    function it_oog_rolls_back_callee_keeps_caller() public {
        Workhorse w = new Workhorse();
        uint beforeCaller = counter;
        counter += 1;
        bool ok;
        try w.mutateThenLoop(7) {
            require(false, "unexpected success");
        } catch {
            ok = true;
        }
        require(ok, "expected revert on OOG");
        require(counter == beforeCaller + 1, "caller change lost on OOG");
        require(w.v() == 0, "callee state not rolled back on OOG");
    }
