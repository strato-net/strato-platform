...
export let protocolContractAddresses: Set<string> = new Set();

export async function initProtocolContractAddresses() {
  ...
  try {
    // Fetch CDP contracts from CDPRegistry
    const { data: cdpRegistryData } = await cirrus.get(
      accessToken,
      `/${constants.CDPRegistry}`,
      {
        params: {
    ...
    if (cdpRegistryData && cdpRegistryData.length > 0) {
    ...
    // Fetch Lending contracts from LendingRegistry
    const { data: lendingRegistryData } = await cirrus.get(
      accessToken,
      `/${constants.LendingRegistry}`,
      {
        params: {
    ...
    if (lendingRegistryData && lendingRegistryData.length > 0) {
      const registry = lendingRegistryData[0];
      ...
      if (collateralVaultAddress) {
        addresses.add(collateralVaultAddress.toLowerCase());
      }
      if (liquidityPoolAddress) {
        addresses.add(liquidityPoolAddress.toLowerCase());
      }
    }
  } catch (error) {
    console.error("Error fetching protocol contract addresses:", error);
    // Continue with empty set - filtering will be less effective but won't break
  }

  protocolContractAddresses = addresses;
}
