...
/**
 * Get token balances only (without collateral)
 */
...
/**
 * Get token collateral only (without balance)
 */
export const getTokenCollateral = async (
  accessToken: string,
  address: string,
  rawParams: Record<string, string | undefined> = {}
) => {
  const [collaterals, cdps, rawPrices] = await Promise.all([
    cirrus.get(accessToken, "/" + CollateralVault + "-userCollaterals", {
      params: {
        select: "user:key,asset:key2,amount:value::text",
        key: `eq.${address}`,
...
/**
 * Get both balance and collateral for a specific token address
 */
export const getTokenValueData = async (
  accessToken: string,
  userAddress: string,
  tokenAddress: string
) => {
  const [balanceData, collaterals, cdps, rawPrices] = await Promise.all([
    cirrus.get(accessToken, "/" + Token + "-_balances", {
      params: {
        key: `eq.${userAddress}`,
        address: `eq.${tokenAddress}`,
        select: tokenBalanceSelectFields.join(","),
      }
    }),
    cirrus.get(accessToken, "/" + CollateralVault + "-userCollaterals", {
      params: {
  ...
  let collateral = 0n;
  [...(collaterals.data || []), ...(cdps.data || [])].forEach((item: any) => {
    collateral += BigInt(item.amount || "0");
  });

  const price = rawPrices.get(tokenAddress) || "0";

  return {
    balance,
    collateralBalance: collateral.toString(),
    price,
  };
};
