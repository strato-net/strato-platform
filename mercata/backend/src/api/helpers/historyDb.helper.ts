...
export interface HistoryDirectTimings {
  pass1SqlMs: number;           // storage + user mapping (parallel)
  computeRelevantTokensMs: number;
  pass2SqlMs: number;           // filtered prices/configs/states
  storageRows: number;
  userMappingRows: number;
  globalMappingRows: number;
  mappingRows: number;          // userMappingRows + globalMappingRows
...
export async function getHistoryDirect(
  ...
  const pass1SqlMs = performance.now() - tPass1Start;
  ...
  const computeRelevantTokensMs = performance.now() - tComputeStart;
  ...
  const pass2SqlMs = performance.now() - tPass2Start;
  ...
  return {
    ...
    timings: {
      pass1SqlMs,
      computeRelevantTokensMs,
      pass2SqlMs,
      storageRows: storageHistory.length,
      userMappingRows: userMappingHistory.length,
      globalMappingRows: globalMappingHistory.length,
      mappingRows: mappingHistory.length,
      relevantTokenCount: relevantTokens.length,
      applyStorageMs,
      applyMappingMs,
      snapshotFnMs,
