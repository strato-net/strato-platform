...
export const contractCall = async (
  ...
  // No authentication required - direct contract calls
  const accessToken = "no_auth_required";
  ...
  const action = async () => {
    const res = await axios.post(`${nodeUrl}/bloc/v2.2/transactions/results`, [txHash], {
      headers: {
        accept: "application/json;charset=utf-8",
        "content-type": "application/json;charset=utf-8",
        authorization: `Bearer ${accessToken}`,
