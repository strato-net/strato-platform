// Safe service utilities for backend bridge operations
// Based on the original bridge service safeService.ts
...
export const checkEthTransactionBatch = async (txList: any[]): Promise<{ txHash: string }[]> => {
  ...
  for (const txHash of txList) {
    try {
      ...
      const transaction = allTxs.results.find(
        (safeTx: any) => txHash === safeTx.transactionHash
      );
      if (transaction) {
        results.push({ txHash: txHash });
      }
    } catch (error) {
