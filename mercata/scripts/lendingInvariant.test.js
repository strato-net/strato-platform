...
(async () => {
  ...
  try {
    ...
    try {
      await sendTx(USER_TOKEN, buildCall("LendingPool", cfg.LENDING_POOL, "withdrawCollateral", { asset: cfg.GOLDST, amount: "1000000000000000000000000000000" }), { quiet: true });
    } catch {}
    ...
    try {
      ...
      if (amt > 0n) {
        try {
          await sendTx(USER_TOKEN, buildCall("LendingPool", cfg.LENDING_POOL, "withdrawCollateral", { asset: cfg.GOLDST, amount: amt.toString() }), { quiet: true });
        } catch {}
      }
    } catch {}

    /* 3. Borrow should fail when user has zero collateral */
    const collCirrus = await getUserCollateralCirrus(cfg.GOLDST);
    ...
    threw = false;
    try {
      await sendTx(ADMIN_TOKEN, buildCall("LendingPool", cfg.LENDING_POOL, "withdrawCollateral", { asset: cfg.GOLDST, amount: "100000000000000000" }), { quiet: true });
    } catch {
      threw = true;
    }
    ...
    const collBefore = await getCollateralOnChain(cfg.GOLDST);
    ...
    const collAfter = await getCollateralOnChain(cfg.GOLDST);
    ...
    let remainingColl = await getCollateralOnChain(cfg.GOLDST);
    if (remainingColl > 0n) {
      try {
        await sendTx(USER_TOKEN, buildCall("LendingPool", cfg.LENDING_POOL, "withdrawCollateral", { asset: cfg.GOLDST, amount: remainingColl.toString() }));
      } catch (err) {
