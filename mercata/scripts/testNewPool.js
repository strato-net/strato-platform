...
(async () => {
  ...
  // Direct transfer USDST to pool before swap
  const TRANSFER_ACCOUNT = "c714b751376045eb67d19ad7329dc0003adfd253";
  const TRANSFER_AMT = "1234567890000000000000";
  console.log(`Transferring ${TRANSFER_AMT} USDST directly to pool from ${TRANSFER_ACCOUNT}...`);
  await contractCall("Token", USDST, "transfer", { to: POOL_ADDR, value: TRANSFER_AMT }, ADMIN_TOKEN);
  console.log("Waiting 1 second for Cirrus to update...");
  await new Promise(resolve => setTimeout(resolve, 1000));
  await recordState("After Direct Transfer");
