...
export const checkBalances = async (): Promise<void> => {
  ...
  if (totalTransactions < config.balance.minTransactionsThreshold) {
    throw new Error(`Total possible transactions (${totalTransactions}) below minimum threshold (${config.balance.minTransactionsThreshold}). Voucher: ${voucherBalanceUSD} (${voucherTransactions} txs), USDST: ${usdstBalanceUSD} (${usdstTransactions} txs)`);
  }
  
  if (totalTransactions < config.balance.warningTransactionsThreshold) {
    logError('BalanceChecker', new Error(`Balance low: Total possible transactions (${totalTransactions}) below warning threshold (${config.balance.warningTransactionsThreshold}). Voucher: ${voucherBalanceUSD} (${voucherTransactions} txs), USDST: ${usdstBalanceUSD} (${usdstTransactions} txs)`));
  }
};
