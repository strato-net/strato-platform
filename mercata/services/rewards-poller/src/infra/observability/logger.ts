import { appendErrorRecord } from "./errorFileSink";
...
const write = async (data: object, isError: boolean) => {
  ...
  if (isError) {
    try {
      const errorJsonString = await appendErrorRecord(data);
      logInfo("HealthMonitor", "Added the error message to the error file", {
        errorJsonString,
      });
    } catch (err) {
      console.error("HealthMonitor: Failed to append the error message to the error file", err);
    }
  }
};
