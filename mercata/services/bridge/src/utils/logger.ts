...
export function logError(
  ...
  if (error instanceof Error && error.stack) {
    console.log(
      `[ERROR] ${logTime} | ${context} | Stack: ${sanitizeLogMessage(error.stack)}`,
    );
  }

  if (additionalData) {
    console.log(
      `[ERROR] ${logTime} | ${context} | Additional Data: ${JSON.stringify(additionalData, null, 2)}`,
    );
  }
}
