...
import { getRebaseFactors, getAssetInfo, getEnabledChains } from "../services/cirrusService";
...
// ABI interfaces for vault and rep bridge contracts
...
export async function createWithdrawalProposals(
  ...
  if (!chainInfo) {
    throw new Error(`Chain ${externalChainId} not found in enabled chains`);
  }
  ...
  for (const withdrawal of withdrawals) {
    ...
    logInfo("SafeService", `Created tx proposal: nonce ${nonce}, withdrawalId ${withdrawal.withdrawalId}, hot: ${!!withdrawal.useHotWallet}, isNative: ${isNative}`);
