...
export async function validateBridgeConfig(): Promise<boolean> {
    ...
    if (process.env.OPENID_DISCOVERY_URL && process.env.CLIENT_ID && process.env.CLIENT_SECRET) {
        try {
            // Test OAuth discovery URL
            const response = await fetch(process.env.OPENID_DISCOVERY_URL);
            if (!response.ok) {
                errors.push(`OAuth discovery failed with status ${response.status}: ${response.statusText}`);
            } else {
                const discovery = await response.json() as any;
                if (!discovery.jwks_uri || !discovery.issuer) {
                    errors.push('OAuth discovery response is invalid - missing jwks_uri or issuer');
                } else {
                    // Test actual user authentication
                    try {
                        const { initOpenIdConfig, getBAUserToken } = await import('../auth');
                        ...
                        errors.push(`User authentication error: ${(authError as Error).message}`);
                    }
                }
            }
        } catch (error) {
    ...
    if (config.bridge.address) {
        if (!/^(0x)?[a-fA-F0-9]{40}$/.test(config.bridge.address)) {
            errors.push(`Invalid bridge contract address format: ${config.bridge.address}`);
        }
    }
    ...
    if (config.safe.safeOwnerAddress) {
        if (!/^(0x)?[a-fA-F0-9]{40}$/.test(config.safe.safeOwnerAddress)) {
            errors.push(`Invalid Safe owner address format: ${config.safe.safeOwnerAddress}`);
        }
    }

    if (config.safe.safeOwnerPrivateKey) {
        if (!/^[a-fA-F0-9]{64}$/.test(config.safe.safeOwnerPrivateKey)) {
            errors.push(`Invalid Safe owner private key format: ${config.safe.safeOwnerPrivateKey.substring(0, 10)}...`);
        }
    }
    ...
    if (config.voucher.contractAddress) {
        if (!/^(0x)?[a-fA-F0-9]{40}$/.test(config.voucher.contractAddress)) {
            errors.push(`Invalid voucher contract address format: ${config.voucher.contractAddress}`);
        }
    }

    // Validate polling intervals
    if (config.polling.bridgeInInterval < 10000) {
        warnings.push('Bridge-in polling interval is very short (< 10s) - may cause rate limiting');
    }

    if (config.polling.bridgeOutInterval < 30000) {
        warnings.push('Bridge-out polling interval is very short (< 30s) - may cause rate limiting');
    }

    if (config.polling.withdrawalInterval < 5000) {
        warnings.push('Withdrawal polling interval is very short (< 5s) - may cause rate limiting');
    }
        ...
        try {
            ...
            for (const chain of enabledChains) {
                ...
                if (!process.env[envVarName]) {
                    ...
                    try {
                        ...
                        warnings.push(`RPC URL for chain ${chainId} is not accessible: ${(error as Error).message}`);
                    }
                }
            }
            
            if (missingChainRpcUrls.length > 0) {
                errors.push(`Missing RPC URL environment variables for enabled chains: ${missingChainRpcUrls.join(', ')}`);
            }
            ...
            for (const asset of enabledAssets) {
                ...
                if (!stratoToken) {
                    ...
                    errors.push(`${assetPrefix} Invalid stratoToken address format: ${stratoToken}`);
                }

                if (!asset.extToken) {
                    errors.push(`${assetPrefix} Missing extToken address`);
                } else if (!/^(0x)?[a-fA-F0-9]{40}$/.test(asset.extToken)) {
                    errors.push(`${assetPrefix} Invalid extToken address format: ${asset.extToken}`);
                }
                ...
                if (!asset.extDecimals) {
                    ...
                    errors.push(`${assetPrefix} Invalid extDecimals format: ${asset.extDecimals}`);
                }
            }

            logInfo('ConfigValidator', `Found ${enabledChains.length} enabled chains and ${enabledAssets.length} enabled assets`);
            
        } catch (error) {
            errors.push(`Failed to validate chain/asset configuration: ${(error as Error).message}`);
        }
    } else {
    ...
    if (warnings.length > 0) {
        logInfo('ConfigValidator', `Configuration warnings:\n${warnings.map(warning => `   ${warning}`).join('\n')}`);
    }
