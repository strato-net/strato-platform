  ...
  try {
    ...
    if (safeTxs?.length > 0) {
      ...
      if (result?.status === "Success") {
        ...
        throw new Error(
          `Failed to confirm withdrawals: ${result?.status || "Unknown error"}`,
        );
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (executedTxs?.length > 0) {
      ...
      if (result?.status === "Success") {
        ...
        throw new Error(
          `Failed to finalize withdrawals: ${result?.status || "Unknown error"}`,
        );
      }
    }
  } catch (error) {
    logError("BridgeService", error as Error, {
      operation: "finaliseWithdrawalBatch",
      withdrawalCount: withdrawals.length,
    });
    throw error;
  }
...
export const handleRejectedWithdrawalBatch = async (withdrawals: any[]) => {
  ...
  try {
    ...
    if (result?.status === "Success") {
      ...
      throw new Error(
        `Failed to abort rejected withdrawals: ${result?.status || "Unknown error"}`,
      );
    }
  } catch (error) {
    logError("BridgeService", error as Error, {
      operation: "handleRejectedWithdrawalBatch",
      withdrawalCount: withdrawals.length,
    });
    throw error;
  }
