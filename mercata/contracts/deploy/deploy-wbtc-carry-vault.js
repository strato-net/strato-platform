/**
 * Deploy a YieldVault for wBTC (ERC-4626 vault accepting wBTC deposits).
 *
 * Steps performed:
 *   1. Deploy YieldVault contract (owner = AdminRegistry)
 *   2. Initialize with wBTC as the underlying asset
 *
 * Usage:
 *   node deploy-wbtc-carry-vault.js
 *
...
// ── Addresses (testnet defaults, overridable via env) ───────────────────
const ADMIN_REGISTRY = process.env.ADMIN_REGISTRY || '000000000000000000000000000000000000100c';
const WBTC_TOKEN     = process.env.WBTC_TOKEN     || '7a99b5ba11ac280cdd5caf52c12fe89fb1b8d2f9';
...
async function call(tokenObj, address, contractName, method, args) {
  ...
  return rest.call(tokenObj, callArgs(address, contractName, method, args), callOpts);
}
...
async function main() {
  try {
    console.log('Deploy wBTC Yield Vault');
    console.log('=======================\n');
    ...
    await saveCreateTXDataAsFile('YieldVault-wBTC', deployResult);
    ...
    console.log(`  2. Set WBTC_CARRY_VAULT=${vaultAddress} in backend .env`);
