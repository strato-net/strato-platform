...
contract Describe_TokenPausable {
    ...
    function it_allows_whitelisted_user_to_transfer_when_paused() {
        ...
        // Set the AdminRegistry in the token
        adminToken.setAdminRegistry(address(adminRegistry));
        
        uint256 transferAmount = 500 * 10**18;  // Reduced amount so user1 has tokens left
        // Have AdminRegistry give user1 some tokens using castVoteOnIssue
        bool success = ERC20(adminToken).transfer(
            address(user1), 
            1000 * 10**18  // Give user1 1000 tokens total
        );
        require(success, "this contract should be able to transfer tokens");

        // Try to have user1 transfer - should succeed since contract is not paused
        success = false;
        ...
        require(success, "User1 should be able to transfer tokens");
        
        // Pause the token using AdminRegistry
        Token(adminToken).pause();
        bool isPaused = Pausable(adminToken).paused();
        require(isPaused, "Admin token should be paused");
        
        // Try to have user1 transfer - should fail since not whitelisted
        success = false;
        try user1.do(address(adminToken), "transfer(address,uint256)", address(user2), transferAmount) {
            success = true;
        } catch {
        ...
        // Now whitelist user1 for transfer function using the token's addWhitelist function
        ...
        // Check if whitelist was set correctly
        bool isWhitelisted = adminRegistry.whitelist(address(adminToken), "_transfer", address(user1));
        require(isWhitelisted, "User1 should be whitelisted for transfer");
        require(Ownable(adminToken).owner() == address(this), "Token owner should be this contract");
        ...
        require(success, "Whitelisted user should be able to transfer when paused");
        
        // Verify the transfer actually happened
        uint256 user2Balance = ERC20(adminToken).balanceOf(address(user2));
        require(user2Balance == 1000 * 10**18, "User2 should have received both transfers");
    }
}