...
contract Describe_CDPEngine {
    ...
    function beforeAll() {
        ...
        usdst = Token(address(cdpRegistry.usdst()));
        usdstAddress = address(usdst);
    }
    ...
    function it_cdp_engine_can_mint_usdst() {
        uint256 depositAmount = 2000e18; // $2000 worth at $1 price
    ...
    function it_cdp_engine_can_mint_max_usdst() {
        uint256 depositAmount = 3000e18; // $3000 worth at $1 price
        ...
        // Verify we're close to liquidation ratio (with buffer)
        uint256 cr = cdpEngine.collateralizationRatio(
            address(this),
            collateralTokenAddress
        );
        require(cr > LIQUIDATION_RATIO, "CR should be above liquidation ratio");
        require(
            cr < LIQUIDATION_RATIO + 10e16,
            "CR should be close to liquidation ratio"
        ); // within 10% buffer
    }
    ...
    function it_cdp_engine_enforces_debt_floor() {
        ...
        // Verify the debt floor exists in configuration
        require(DEBT_FLOOR == 100e18, "Debt floor should be 100 USD");
    }
