import "../../concrete/Bridge/BridgeCore.sol";
...
contract Describe_BridgeCore is Authorizable {
    ...
    function beforeEach() {
        ...
        bridge = new BridgeCore(address(adminRegistry));
    ...
    function it_bridge_reverts_with_zero_addresses() {
        bool reverted = false;
        try {
            new BridgeCore(owner).initialize(address(0), address(relayer));
        } catch {
            reverted = true;
        }
        ...
        reverted = false;
        try {
            new BridgeCore(owner).initialize(address(tokenFactory), address(0));
        } catch {
            reverted = true;
        }
    ...
    function it_bridge_can_confirm_withdrawal() {
        uint256 amount = 1000e18;
        address recipient = address(0xEEEE);
        string memory custodyTxHash = "custody123";
    ...
    function it_bridge_handles_different_chain_ids() {
        ...
        bridge.deposit(chainId1, externalSender, address(0x5555), "0x1a2b3c4d5e6f", address(0x1111), 1000e18);
        bridge.deposit(chainId2, externalSender, address(0x8888), "0xf6e5d4c3b2a1", address(0x2222), 2000e18);
    ...
    function it_bridge_reverts_deposit_with_inactive_token() {
        ...
        bool reverted = false;
        try {
            bridge.deposit(externalChainId, externalSender, address(0x7777), "1n4ct1v3", address(0x1111), 1000e18);
        } catch {
            reverted = true;
        }
    ...
    function it_bridge_reverts_finalise_withdrawal_in_wrong_state() {
        ...
        bool reverted = false;
        try {
            bridge.finaliseWithdrawal(withdrawalId, "custody");
        } catch {
            reverted = true;
        }
    ...
    function it_bridge_reverts_abort_withdrawal_in_wrong_state() {
        ...
        bridge.confirmWithdrawal(withdrawalId, "custody");
        bridge.finaliseWithdrawal(withdrawalId, "custody");
