...
contract Describe_Mercata is Authorizable {
    ...
    function it_can_make_money_on_a_one_sided_deposit() {
        uint a = 3; // Pool liquidity: 3 ETH
        uint b = 10500; // Pool liquidity: $10,500 ($3,500/ETH)
        uint c = 5500; // Deposit: $5,500 (>50%)
        address t1 = m.tokenFactory().createToken("ETHST", "ETHST Token", [], [], [], "ETHST", 0, 18);
        Token(t1).setStatus(2);
        address t2 = m.tokenFactory().createToken("USDST", "USDST Token", [], [], [], "USDST", 0, 18);
        Token(t2).setStatus(2);
        ...
        uint u1t1After = Token(t1).balanceOf(address(u1));
        uint u1t2After = Token(t2).balanceOf(address(u1));
        uint totalAfter = ((t2Amt*u1t1After)/t1Amt) + u1t2After;
        int percentageGain = (100000000*(((int(t2Amt)*int(u1t1After)) / int(t1Amt)) + int(u1t2After)) / int(u1Amt)) - 100000000;
        log("Percentage gain: " + string(percentageGain/1000000) + "." + string(percentageGain % 1000000) + "% : " + string(a) + " " + string(b) + " " + string(c));
        ...
        require(totalAfter > u1Amt, "Liquidity deposit yielded profit: (" + string(t2Amt/t1Amt) + "*" + string(u1t1After) + ") + " + string(u1t2After) + " = " + string(totalAfter));
    }

    function xproperty_can_make_money_on_a_one_sided_deposit(uint _a, uint _b1, uint _b2, uint _b3, uint _c) {
        uint a = _a + 1;
        uint b = a * (10 + (((_b1 % 10) + 1) * ((_b2 % 10) + 1) * ((_b3 * 10) + 1)));
        uint c = ((b/1000) + 1) * ((_c % 2000) + 1) + 12;
        address t1 = m.tokenFactory().createToken("ETHST", "ETHST Token", [], [], [], "ETHST", 0, 18);
        Token(t1).setStatus(2);
        address t2 = m.tokenFactory().createToken("USDST", "USDST Token", [], [], [], "USDST", 0, 18);
        Token(t2).setStatus(2);
    ...
    function it_keeps_going_1() {
        uint a = 3; // Pool liquidity: 3 ETH
        uint b = 10500; // Pool liquidity: $10,500 ($3,500/ETH)
        uint c = 10500; // Deposit: $5,500 (>50%)
        address t1 = m.tokenFactory().createToken("ETHST", "ETHST Token", [], [], [], "ETHST", 0, 18);
        Token(t1).setStatus(2);
        address t2 = m.tokenFactory().createToken("USDST", "USDST Token", [], [], [], "USDST", 0, 18);
        Token(t2).setStatus(2);
        ...
        uint i = 0;
        while (i < 50) { // $0.10
            u1t1After = Token(t1).balanceOf(address(u1));
            u1t2After = Token(t2).balanceOf(address(u1));
            uint p1t1After = Token(t1).balanceOf(address(p1));
            uint p1t2After = Token(t2).balanceOf(address(p1));
            uint ptotalAfter = ((t2Amt*p1t1After)/t1Amt) + p1t2After;
            totalAfter = ((t2Amt*u1t1After)/t1Amt) + u1t2After;
            percentageGain = (100000000*(((int(t2Amt)*int(u1t1After)) / int(t1Amt)) + int(u1t2After)) / int(u1Amt)) - 100000000;
            log("Results after " + string(i) + " rounds");
            log("Pool's ETH balance:   " + string(p1t1After/1e18) + "." + string((p1t1After/1e15) % 1000));
            log("Pool's USD balance:   " + string(p1t2After/1e18) + "." + string((p1t2After/1e15) % 1000));
            log("Pool's total balance: " + string(ptotalAfter/1e18) + "." + string((ptotalAfter/1e15) % 1000));
            log("User's ETH balance:   " + string(u1t1After/1e18) + "." + string((u1t1After/1e15) % 1000));
            log("User's USD balance:   " + string(u1t2After/1e18) + "." + string((u1t2After/1e15) % 1000));
            log("User's total balance: " + string(totalAfter/1e18) + "." + string((totalAfter/1e15) % 1000));
            log("Percentage gain:      " + string(percentageGain/1000000) + "." + string(percentageGain % 1000000) + "%");
            uint haha = lol - 1e17;
            require(u1.do(t2, "approve", p1, haha), "Approval failed for u1");
            u1.do(p1, "addLiquiditySingleToken", false, haha, 1);
            uint lpBal = lpToken.balanceOf(address(u1));
            log("User's LP balance:    " + string(lpBal/1e18) + "." + string((lpBal/1e15) % 1000));
            log("User's LP share:      " + string((lpBal*100)/lpToken.totalSupply()) + "." + string(((lpBal*100000)/lpToken.totalSupply()) % 1000));
            log("--------------------------------------------");
            u1.do(p1, "removeLiquidity", lpBal, 1, 1, 1);
            //uint lmao = Token(t1).balanceOf(address(u1));
            //require(u1.do(t1, "approve", p1, lmao), "Approval failed for u1");
            //u1.do(p1, "swap", true, lmao, 1, 1);
            lol = Token(t2).balanceOf(address(u1));
            i++;
        }
        // uint lmao = Token(t1).balanceOf(address(u1));
        // require(u1.do(t1, "approve", p1, lmao), "Approval failed for u1");
