...
contract Describe_Mercata {
    ...
    function it_can_execute_internal_issues() {
        ...
        User u = new User();
        AdminRegistryV1 admin = new AdminRegistryV1([this, address(adminUser)]);
        admin.castVoteOnIssue(address(admin), "addAdmin", address(u));
        require(admin.admins().length == 2, "Admin was added before enough votes were cast");
        adminUser.do(address(admin), "castVoteOnIssue", address(admin), "addAdmin", address(u));
    ...
    function it_can_change_voting_logic() {
        ...
        admin.castVoteOnIssue(address(admin), "removeAdmin", address(adminUser));
