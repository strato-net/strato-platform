...
function queryNetworkHealthStatus() {
    return new Promise(async (resolve, _void) => {
      try {
        winston.info(`Retrieving network health from Monitor Host: ${MONITOR_URL}`)
        const status = await getNetworkStatus();
        winston.debug(`Network is ${status.needsAttention ? 'unhealthy' : 'healthy'}`);
        winston.info('Updating network health')
        await updateNetworkHealthStatus(status);
        winston.info('Network health updated')
        return resolve();
      } catch (error) {
        winston.error(`Error occurred while querying network health: "${error.message ? error.message : 'no message'}"`);
        return resolve();
      }
...
async function updateNetworkHealthStatus(status){
    ...
    let [stat, created] = await models.CurrentHealth.findOrCreate({where: {processName: 'NetworkHealthStat'}, 
        defaults: {
        ...
        await stat.update(
            {
                latestCheckTimestamp: currentTime,
                latestHealthStatus: !status.needsAttention,
                lastFailureTimestamp: !status.needsAttention ? stat.lastFailureTimestamp : currentTime,
                additionalInfo: JSON.stringify(status),
            },
            {
