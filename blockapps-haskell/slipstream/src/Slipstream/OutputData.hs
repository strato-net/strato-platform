...
createInserts globalsIORef = do
  ...
  when (not $ contractAlreadyCreated) $ do
      let conVals = T.concat ["('", (codehash firstContract), "', '", (contractName firstContract), "', '", (abi firstContract), "', '", (chain firstContract), "')"]
  ...
  if (length metadata > 1)
    then do
      ...
      let ins = T.concat ["insert into \"", (contractName firstContract), "\" ", keySt, " values ", inserts, " on conflict (address, \"chainId\") do update set address = excluded.address, \"chainId\" = excluded.\"chainId\"", comma, (tableUpsert list), ";"]

      yield ins
  else do
    let vals = T.concat ["(", "'", address firstContract, "', '", chain firstContract, "'", comma , listToValueStatement ", " list, ")"]
    let ins = T.concat ["insert into \"", tableName, "\" ", keySt, " values ", vals, " on conflict (address, \"chainId\") do update set address = excluded.address, \"chainId\" = excluded.\"chainId\"", comma, (tableUpsert list), ";"]
    yield ins
  ...
  viewSt <- if(resolvedName newCachedContract == Just (T.concat [name newCachedContract, "1"]))
      then do
        return $ T.concat ["create or replace view \"", name newCachedContract, "\" as select * from \"", name newCachedContract, "1\";"]
      else do
        --Get the first contract that uses the name
        let originalSchemaContract = filter (\(_, y) -> resolvedName y == Just (T.concat [officialName, "1"])) newCacheList
        ...
        let tableString = map (\table -> T.concat ["UNION SELECT * FROM \"", fromMaybe (name newCachedContract) (resolvedName $ snd table), "\""]) tableList

        return $ T.concat ["create or replace view \"", (name newCachedContract), "\" as select * from \"", (name newCachedContract), "1\" ", (T.intercalate " " tableString), ";"]
