...
module.exports = function(config) {
  ...
  return {
    ...
    strato: {
      ...
      searchHash: function(codeHash) {
        return ax.get(config.getStratoUrl(node), '/eth/v1.2/account?codeHash=' + codeHash);
      }
    },
    ...
    bloc: {
      ...
      sendList: function(body, from, address){
        return ax.post(config.getBlocUrl(node), body, '/users/' + from + '/' + address + '/sendList');//.then(function(x){Promise.all(x.map(rfc2616Tx))});
      },
      ...
      state: function(name, address, lookup) {
      ...
      compile: function(contractSources) {
        return ax.post(config.getBlocUrl(node), contractSources, '/contracts/compile');
      }
