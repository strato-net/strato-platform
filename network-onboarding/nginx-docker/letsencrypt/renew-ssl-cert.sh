...
docker stop network-onboarding_http-redirect_1
...
function cleanup {
  docker-compose down
  docker start network-onboarding_http-redirect_1
}
...
if [[ "${DRY_RUN}" = "true" ]]; then
  ...
  echo "Copying key and cert to the network-onboarding nginx ssl dir..."
  set -x
  ...
  set -x
  sudo docker cp --follow-link ${cert_path} network-onboarding_nginx_1:/etc/ssl/server.pem
  sudo docker cp --follow-link ${key_path} network-onboarding_nginx_1:/etc/ssl/server.key
  ...
  set -x
  sudo docker exec network-onboarding_nginx_1 nginx -s reload
