...
if [ "${IS_BOOTNODE}" = "false" ]; then
  if [ ! -f "${CONFIG_DIR_PATH}/${DEPLOY_FILE_NAME}" ]; then
    echo "IS_BOOTNODE=false but there was no ${DEPLOY_FILE_NAME} provided in network-onboarding_config docker volume. Exit."
    exit 53
  fi
else
...
echo 'Starting network-onboarding-server...'
yarn start:prod
