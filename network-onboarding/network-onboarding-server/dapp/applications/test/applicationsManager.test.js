...
import applicationsManagerJs from '/dapp/applications/applicationsManager'
...
describe('Application Manager', function () {
  ...
  it('Create Application Manager', async () => {
    ...
    const contract = await applicationsManagerJs.uploadContract(networkAdmin, {
      permissionManager: permissionManagerContract.address,
  ...
  describe('Application Creation', () => {
    ...
    before(async () => {
      adminContract = await applicationsManagerJs.uploadContract(networkAdmin, {
        permissionManager: permissionManagerContract.address,
        organizationsManager: organizationsManagerContract.address,
      }, options)

      orgAdminContract = await applicationsManagerJs.uploadContract(orgAdmin, {
        permissionManager: permissionManagerContract.address,
        organizationsManager: organizationsManagerContract.address,
      }, options)
    ...
    it.skip('Create Application - 201 - CREATED', async () => {
      ...
      // create Organization
      const result = await organizationsManagerContract.createOrganization({
        userCertificate: '-----BEGIN CERTIFICATE-----\\n0t1hHbhYnJarPkNA\n-----END CERTIFICATE-----'
      })
