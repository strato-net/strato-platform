...
import userManagerJs from '/dapp/users/networkOnboardingUsersManager'
import organizationsManagerJs from '/dapp/organizations/organizationsManager'
import organizationMembershipsManagerJs from '/dapp/organizations/membership/organizationMembershipsManager'
import organizationMembershipJs from '/dapp/organizations/membership/organizationMembership'
...
describe('OrganizationMembership Manager', function () {
  ...
  let organizationsManagerContract
  let networkAdmin
  let orgAdmin
  let orgUser
  let enodeAddress
  ...
  before(async () => {
    ...
    organizationsManagerContract = await organizationsManagerJs.uploadContract(networkAdmin, {
      permissionManager: permissionManagerContract.address,
  ...
  it('Create OrganizationMembership Manager', async () => {
    const contract = await organizationMembershipsManagerJs.uploadContract(networkAdmin, {
      permissionManager: permissionManagerContract.address,
      organizationsManager: organizationsManagerContract.address,
    }, options)
    ...
    assert.equal(organizationsManager, organizationsManagerContract.address, 'organizationsManager')
  })
  ...
  it('Request OrganizationMembership', async () => {
    ...
    const userBindedContract = organizationMembershipsManagerJs.bindAddress(orgAdmin, contract.address, options)

    const organizationMembershipArgs = {
      ...factory.getOrganizationMembershipsManagerArgs(util.uid()),
      enodeAddress,
    }
  ...
  describe('Reject organizationMembership', () => {
    ...
    before(async () => {
      contract = await organizationMembershipsManagerJs.uploadContract(networkAdmin, {
        permissionManager: permissionManagerContract.address,
        organizationsManager: organizationsManagerContract.address,
      }, options)
      const userBindedContract = organizationMembershipsManagerJs.bindAddress(orgUser, contract.address, options)
      const organizationMembershipArgs = {
        ...factory.getOrganizationMembershipsManagerArgs(util.uid()),
        enodeAddress,
      }
  ...
  describe.skip('Accept organizationMembership', () => {
    let contract
    let organizationMembershipContractAddress
