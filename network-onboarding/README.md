...
### Run network-onboarding application locally (for development)
...
```
cd network-onboarding/nginx-docker
```
...
```
cd network-onboarding/network-onboarding-server
yarn install
```
...
```
cd network-onboarding/network-onboarding-ui
yarn install
yarn develop
```
...
### Run network-onboarding app in Docker (the production way)
...
#### 2a. Run as network-onboarding bootnode (main node in multi-node environment)
...
1. On bootnode - Get deploy file content:
    ```
    sudo docker exec network-onboarding_backend_1 cat /config/network-onboarding.deploy.yaml
    ```
2. On secondary node - create docker volume and add the same network-onboarding.deploy.yaml file using commands:
    ```
    sudo su
    docker volume create network-onboarding_config
    APP_CONFIG_VOLUME_DIR=$(docker volume inspect --format '{{ .Mountpoint }}' network-onboarding_config)
    nano ${APP_CONFIG_VOLUME_DIR}/network-onboarding.deploy.yaml
...
DEPLOY_FILE_NAME            - (default: 'network-onboarding.deploy.yaml') filename of the targeted deploy file. Not recommended to change unless you know what you are doing.
...
NODE_LABEL                  - (required) String representing the node identificator (e.g. network-onboarding-node1)
...
OAUTH_APP_TOKEN_COOKIE_NAME - (default: 'network-onboarding_session') Browser session cookie name for the node, e.g. network-onboarding-node1-session'
...
For steps to use letsencrypt tool please refer to network-onboarding/nginx-docker/letsencrypt/README.md
...
To use this utility, run `sudo yarn token-getter` from the `network-onboarding-server` directory:
...
```
cd network-onboarding/network-onboarding-selenium
yarn test:selenium
```
