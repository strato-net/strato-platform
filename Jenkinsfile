    ...
    stage('Deploy') {
      steps {
        sh '''#!/bin/bash -le
          ...
          genesisBlock=$(< gb.json) explorerHost=http://explorer:9000 explorerAdvertise=http://localhost stratoHost=nginx stratourl=nginx/strato-api cirrusurl=nginx/cirrus ssl=false docker-compose up -d
          docker ps
        '''
      }
    }
