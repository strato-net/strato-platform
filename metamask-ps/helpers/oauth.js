import { getEmailIdFromToken, getServiceToken, createStratoUser } from "./oauthHelper.js";
...
class Admin {
  ...
  async bootstrap() {
    ...
    let tokenExpiration;
    try {
      const { token, expiration } = await getServiceToken();
    ...
    const adminEmail = await getEmailIdFromToken(serviceUserToken);

    console.log("Creating Admin...", adminEmail);
    const adminCredentials = { token: serviceUserToken };
    const adminResponse = await createStratoUser(
      adminCredentials,
      adminEmail
    );
