...
.PHONY: strato apex highway highway-nginx nginx postgrest prometheus smd marketplace-backend marketplace-ui vault-wrapper vault-nginx identity-provider identity-nginx payment-server payment-server-nginx build_buildbase build_common build_common_profiled eks
...
docker-compose:
	...
	sed -e 's|<REPO_URL>|'"${REPO_URL}"'|g' -e 's|<VERSION>|'"${VERSION}"'|g' docker-compose.payment-server.tpl.yml > docker-compose.payment-server.push.yml
	sed -e 's|<REPO_URL>|'"${REPO_AWS_ECR_URL}"'|g' -e 's|<VERSION>|'"${VERSION}"'|g' docker-compose.payment-server.tpl.yml > docker-compose.payment-server.push.ecr.yml
	...
	awk '/build: ./{getline} 1' docker-compose.payment-server.push.yml > docker-compose.payment-server.yml
	awk '/build: ./{getline} 1' docker-compose.payment-server.push.ecr.yml > docker-compose.payment-server.ecr.yml
