...
const BridgePage = () => {
  ...
  const mapping = {
    [NATIVE_TOKEN_ADDRESS]: BRIDGE_TOKEN_ADDRESS,
    [TOKEN_ADDRESSES["Sepolia"]?.["USDC"]]: BRIDGE_TOKEN_ADDRESS,
  };
    ...
    try {
      ...
      if (fromChain === "STRATO" && toChain !== "STRATO") {
        ...
        const response = await BRIDGE_API.bridgeOut({
          ...
          tokenAddress: mapping[NATIVE_TOKEN_ADDRESS],
        });
  ...
  const handleBridgeIn = async () => {
    try {
      ...
      try {
        ...
        try {
          const bridgeResponse = await BRIDGE_API.bridgeIn({
            amount,
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      if (fromChain === "STRATO" && fromToken && address) {
        ...
        try {
          let tokenAddress;
          console.log("fromToken", fromToken, BRIDGE_TOKEN_ADDRESS);
