...
main = hspec $ do
    ...
    around withConn $ do
    describe "some hedis tests" $ do
       it "should..." $ \c -> do
          fromHUnitTest <$> tests c

--main = do
--    conn <- connect defaultConnectInfo
--    Test.defaultMain (tests conn)

type Test = Connection -> HUnit.Test
...
tests conn = map ($conn) $ concat
    ...
    , testsZSets, [testTransaction]
    , testsConnection, testsServer 
  ...
  where
    go = do
        ...
        (liftIO $ do
            ...
            mvar <- newEmptyMVar
            _ <- forkIO $ runRedis conn (get "key") >>= putMVar mvar
            takeMVar mvar)
          >>=? Just "value"
