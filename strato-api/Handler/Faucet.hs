...
emitKafkaTransactions txs = do
    ...
    $logInfoS "writeUnseqEventsBegin" . T.pack $ "Writing " ++ (show length ingestTxs) ++ " faucet tx(s) to unseqevents"    
    ...
    case rets of
        ...
        Right resps -> $logInfo $ "writeUnseqEventsEnd Kafka commit: " Import.++ (T.pack $ show resps)
    return ()
