...
if [ ! -f "${APP_CONFIG_VOLUME_DIR}/tCommerce.deploy.yaml" ] 
  then
    echo "tCommerce.deploy.yaml is missing from ${APP_CONFIG_VOLUME_DIR} - please make sure it exists before running this script...";
    exit 1;
fi
...
export NODE_LABEL='tcommerce_secondary'
