...
contract ProductManager is UnitOfMeasurement, InventoryStatus,RestStatus{
    ...
    function addInventory (address _productAddress, int _quantity, int _pricePerUnit, string _batchId, 
        InventoryStatus _status, uint _createdDate, string[] _serialNumbers, uint _taxPercentageAmount, uint _taxDollarAmount) returns (uint256, address) {
        if (_serialNumbers.length == 0) {
            Product_3 product = Product_3(_productAddress);
            return product.addInventory(_quantity, _pricePerUnit, _batchId, _status, _createdDate,tx.origin, _taxPercentageAmount, _taxDollarAmount);
