...
const contractFilename = `${util.cwd}/dapp/products/contracts/Carbon.sol`;
/** 
 * Upload a new Carbon 
 * @param user User token (typically an admin)
 * @param _constructorArgs Arguments of Carbon's constructor
 ...
 * 
 * As our arguments come into the carbon contract they first pass through `marshalIn` and 
 ...
 * 
 * As our arguments come into the carbon contract they first pass through {@link marshalIn `marshalIn`} 
...
/**
 * Bind functions relevant for carbon to the _contract object. 
 * @param user User token
 * @param _contract Contract object from `rest.createContract()` etc.
 * @param options Carbon deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
 ...
 * the same contract.
 * @example <caption>Create an admin and user bound to the same new carbon contract.</caption>
 ...
 * @param user User token
 * @param address Address of the Carbon contract
 * @param options Carbon deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
...
async function get(user, args, options) {
    ...
    carbon = await searchOne(contractName, searchArgs, options, user);
...
async function getAll(admin, args = {}, options) {
    const carbons = await searchAllWithQueryArgs(contractName, args, options, admin)
    return carbons.map((carbon) => marshalOut(carbon))
}
