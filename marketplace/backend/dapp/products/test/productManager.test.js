...
describe('Product Manager', function () {
    ...
    it('Create a product', async () => {
        ...
        const [restStatus, productAddress] = await contract.createProduct({ ...args.productArgs });

        // Check if Product was created
        const product = await contract.getProduct({ address: productAddress }, newOptions)
    ...
    it('Create and delete the product', async () => {
        ...
        // delete the product
        const deleteResponse = await contract.deleteProduct({ productAddress: productAddress });
    ...
    it('Create and update inventory for product', async () => {
        ...
        const updateArgs = updateinventoryFactoryArgs(productAddress, inventoryResponse[1]);
    ...
    it('create products (multiple)', async () => {
        ...
        const [status1, productAddress1] = await contract.createProduct({ ...args1.productArgs });
        const [status2, productAddress2] = await contract.createProduct({ ...args2.productArgs });
        const [status3, productAddress3] = await contract.createProduct({ ...args3.productArgs });
        const [status4, productAddress4] = await contract.createProduct({ ...args4.productArgs });

        const productData1 = await contract.getProduct({ address: productAddress1 }, newOptions);
        const productData2 = await contract.getProduct({ address: productAddress2 }, newOptions);
        const productData3 = await contract.getProduct({ address: productAddress3 }, newOptions);
        const productData4 = await contract.getProduct({ address: productAddress4 }, newOptions);

        // Our logic shouldn't mix up products
        assert.deepInclude(R.map(v => '' + v, productData1), R.map(v => '' + v, { ...args1.productArgs }));
        assert.deepInclude(R.map(v => '' + v, productData2), R.map(v => '' + v, { ...args2.productArgs }));
