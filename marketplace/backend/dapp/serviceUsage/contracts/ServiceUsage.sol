...
contract ServiceUsage is Status, PaymentStatus {
    ...
    function changeStatus(Status newType) public {
      if(newType == Status.REQUESTED || newType == Status.COMPLETED || newType == Status.CANCELLED){ // Add more here in the future
        status = newType;
      }
    }
    function changePaymentStatus(PaymentStatus newSection) public {
      if(newSection == PaymentStatus.PAID || newSection == PaymentStatus.UNPAID){ // Add more here in the future
