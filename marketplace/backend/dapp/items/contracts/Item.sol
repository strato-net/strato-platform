...
contract Item_3 is ItemStatus{
    ...
    event OwnershipUpdate(string seller, string newOwner, uint ownershipStartDate, address itemAddress, string appChainId);
    ...
    constructor(
        ...
        if(_rawMaterialSerialNumber.length > 0){
          addRawMaterials(_uniqueProductCode, _rawMaterialProductName, _rawMaterialSerialNumber,_rawMaterialProductId);
        }
    }
    ...
    function generateOwnershipHistory(string _seller, string _newOwner, uint _ownershipStartDate, address _itemAddress) returns (uint){
       if(tx.origin != owner){
         return RestStatus.FORBIDDEN;
       }
        emit OwnershipUpdate(_seller, _newOwner, _ownershipStartDate, _itemAddress, appChainId);
          return RestStatus.OK;
    }

    // Transfer the ownership of a Item
    function transferOwnership(address _addr, address _productId, address _inventoryId) public returns (uint256) {
      ...
      generateOwnershipHistory(ownerOrganization,newOwnerOrganization, block.timestamp,address(this));
    ...
    function addRawMaterial(uint _uniqueProductCode, string _rawMaterialProductName, string _rawMaterialSerialNumber, string _rawMaterialProductId) public returns(uint256){

      RawMaterial_3 rawMaterial = new RawMaterial_3(appChainId, serialNumber, _rawMaterialSerialNumber, _rawMaterialProductName, _uniqueProductCode, _rawMaterialProductId, createdDate);
      return RestStatus.OK;
    }

    // Add the raw materials for the item
    function addRawMaterials(uint _uniqueProductCode, string[] _rawMaterialProductName, string[] _rawMaterialSerialNumber, string[] _rawMaterialProductId) public returns(uint256){
      for (uint256 i = 0; i < _rawMaterialProductName.length; i++) {
          addRawMaterial(_uniqueProductCode, _rawMaterialProductName[i], _rawMaterialSerialNumber[i], _rawMaterialProductId[i]);
      }
      return RestStatus.OK;
    }
