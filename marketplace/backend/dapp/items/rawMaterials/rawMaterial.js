...
/** 
 * Upload a new Raw Material 
 * @param user User token (typically an admin)
 * @param _constructorArgs Arguments of RawMaterial's constructor
 ...
 * 
 * As our arguments come into the rawMaterial contract they first pass through `marshalIn` and 
 ...
 * 
 * As our arguments come into the rawMaterial contract they first pass through {@link marshalIn `marshalIn`} 
...
/**
 * Bind functions relevant for rawMaterial to the _contract object. 
 * @param user User token
 * @param _contract Contract object from `rest.createContract()` etc.
 * @param options RawMaterial deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
...
function bind(user, _contract, options) {
    ...
    contract.getAll = async () => getAll(user, args, options);
...
/** 
 * Bind an existing RawMaterial contract to a new user token. Useful for having multiple users test
 * the same contract.
 * @example <caption>Create an admin and user bound to the same new rawMaterial contract.</caption>
 ...
 * @param user User token
 * @param address Address of the RawMaterial contract
 * @param options RawMaterial deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
 ...
 * @param args Lookup with an address or uniqueRawMaterialID.
...
async function get(user, args, options) {
    const { uniqueRawMaterialID, address, ...restArgs } = args;
    let rawMaterial;

    if (address) {
        const searchArgs = setSearchQueryOptions(restArgs, { key: 'address', value: address });
        rawMaterial = await searchOne(contractName, searchArgs, options, user);
    } else {
        const searchArgs = setSearchQueryOptions(restArgs, { key: 'uniqueRawMaterialID', value: uniqueRawMaterialID });
        rawMaterial = await searchOne(contractName, searchArgs, options, user);
    }
    if (!rawMaterial) {
        return undefined;
...
async function getAll(user, args = {}, options) {
    const rawMaterials = await searchAllWithQueryArgs(contractName, args, options, user)
    return rawMaterials.map((rawMaterial) => marshalOut(rawMaterial))
}
