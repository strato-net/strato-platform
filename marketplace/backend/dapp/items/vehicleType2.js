...
const contractName = "VehicleType2";
const contractFilename = `${util.cwd}/dapp/items/contracts/VehicleType2.sol`;
const contractEvents = { OWNERSHIP_UPDATE: "OwnershipUpdate" }

/** 
 * Uploads a new VehicleType2 item 
...
/** 
 * Bind an existing VehicleType2 contract to a new user token. Useful for having multiple users test
 * the same contract.
...
async function getAll(admin, args = {}, options) {
    const inventories = await searchAllWithQueryArgs(constants.assetTableName, { ...args, category: `['VehicleType2']` }, options, admin);
