...
const contractName = "Clothing";
const contractFilename = `${util.cwd}/dapp/items/contracts/Clothing.sol`;
const contractEvents = { OWNERSHIP_UPDATE: "OwnershipUpdate" }

/** 
 * Uploads a new Clothing item 
...
/** 
 * Bind an existing Clothing contract to a new user token. Useful for having multiple users test
 * the same contract.
...
async function getAll(admin, args = {}, options) {
    const inventories = await searchAllWithQueryArgs(constants.assetTableName, { ...args, category: `['Clothing']` }, options, admin);
...
async function updateClothing(user, contract, args, options) {
    const callArgs = {
      contract,
