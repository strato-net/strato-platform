...
const contractName = "AssetGroup";
const contractFilename = `${util.cwd}/dapp/mercata-base-contracts/Templates/Assets/AssetGroup.sol`;
const contractEvents = { OWNERSHIP_UPDATE: "OwnershipUpdate" }

/** 
 * Uploads a new Asset Group item 
...
/** 
 * Bind an existing AssetGroup contract to a new user token. Useful for having multiple users test
 * the same contract.
 * @example <caption>Create an admin and user bound to the same new item contract.</caption>
 * const adminBoundContract = createAssetGroup(adminToken, args, options);
