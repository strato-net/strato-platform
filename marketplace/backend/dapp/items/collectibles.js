...
/**
 * Uploads a new Collectible item
...
/**
 * Bind an existing Collectible contract to a new user token. Useful for having multiple users test
 * the same contract.
...
async function updateCollectible(user, contract, args, options) {
  const callArgs = {
    contract,
    method: 'updateCollectible',
    args: util.usc({ ...args }),
  };
