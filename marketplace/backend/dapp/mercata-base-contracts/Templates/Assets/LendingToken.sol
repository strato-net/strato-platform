...
abstract contract LendingToken is Mintable, MinterAuthorization {
    ...
    function mintNewUnits(uint _quantity) public override returns (uint) {
        ...
        require(_quantity > 0, "Quantity must be greater than 0");
        require(getCommonName(msg.sender) == minterCommonName || MinterAuthorization(address(this)).isReserveMinter(msg.sender), "Only one of the minter can mint new units");
