...
const contractName = 'Category';
const contractFilename = `${util.cwd}/dapp/categories/contracts/Category.sol`;

/** 
 * Upload a new Category 
 * @param user User token (typically an admin)
 * @param _constructorArgs Arguments of Category's constructor
 ...
 * 
 * As our arguments come into the category contract they first pass through `marshalIn` and 
 ...
 * 
 * As our arguments come into the category contract they first pass through {@link marshalIn `marshalIn`} 
...
/**
 * Bind functions relevant for category to the _contract object. 
 * @param user User token
 * @param _contract Contract object from `rest.createContract()` etc.
 * @param options Category deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
...
/** 
 * Bind an existing Category contract to a new user token. Useful for having multiple users test
 * the same contract.
 * @example <caption>Create an admin and user bound to the same new category contract.</caption>
 ...
 * @param user User token
 * @param address Address of the Category contract
 * @param options Category deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
 ...
 * @param args Lookup with an address or uniqueCategoryID.
...
async function get(user, args, options) {
    ...
    if (address) {
        ...
        category = await searchOne(contractName, searchArgs, options, user);
    } else {
    ...
    return marshalOut({ ...category, 
    });
}

async function getAll(admin, args = {}, options) {
    const categorys = await searchAllWithQueryArgs(contractName, args, options, admin)
    return categorys.map((category) => marshalOut(category))
}
