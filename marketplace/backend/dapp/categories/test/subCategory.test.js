...
import factory from '../factory/subCategory.factory';
...
describe('Sub Category', function() {
    ...
    const factoryArgs = (user) => ({ ...(factory.getSubCategoryArgs(util.uid()))});
    ...
    it('should upload SubCategory smart contract', async () => {
    const args=factoryArgs(globalAdmin)
    contract=await subCategoryJS.uploadContract(globalAdmin,args,newOptions);
    const state = await contract.get({address:contract.address})

    assert.deepInclude(
        // Convert the subCategory data into strings as the args are in strings
        R.map(v => '' + v, state),
        R.map(v => '' + v, args));
    });
