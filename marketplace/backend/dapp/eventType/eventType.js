...
/** 
 * Upload a new EventType 
 * @param user User token (typically an admin)
 * @param _constructorArgs Arguments of EventType's constructor
...
/**
 * Bind functions relevant for eventType to the _contract object. 
 * @param user User token
 * @param _contract Contract object from `rest.createContract()` etc.
 * @param options EventType deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
...
/** 
 * Bind an existing EventType contract to a new user token. Useful for having multiple users test
 * the same contract.
 * @example <caption>Create an admin and user bound to the same new eventType contract.</caption>
 ...
 * @param user User token
 * @param address Address of the EventType contract
 * @param options EventType deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
 ...
 * @param args Lookup with an address or uniqueEventTypeID.
...
async function get(user, args, options) {
    ...
    if (address) {
        ...
        eventType = await searchOne(contractName, searchArgs, options, user);
    } else {
