 ...
 * @param _constructorArgs Arguments of OrderLineItem's constructor
 ...
 * 
 * As our arguments come into the orderLineItem contract they first pass through `marshalIn` and 
 ...
 * 
 * As our arguments come into the orderLineItem contract they first pass through {@link marshalIn `marshalIn`} 
...
/**
 * Bind functions relevant for orderLineItem to the _contract object. 
 * @param user User token
 * @param _contract Contract object from `rest.createContract()` etc.
 * @param options OrderLineItem deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
...
/** 
 * Bind an existing OrderLineItem contract to a new user token. Useful for having multiple users test
 * the same contract.
 * @example <caption>Create an admin and user bound to the same new orderLineItem contract.</caption>
 ...
 * @param user User token
 * @param address Address of the OrderLineItem contract
 * @param options OrderLineItem deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
 ...
 * @param args Lookup with an address or uniqueOrderLineItemID.
...
async function get(user, args, options) {
    const { uniqueOrderLineItemID, address, ...restArgs } = args;
    let orderLineItem;

    if (address) {
        const searchArgs = setSearchQueryOptions(restArgs, { key: 'address', value: address });
        orderLineItem = await searchOne(contractName, searchArgs, options, user);
    } else {
        const searchArgs = setSearchQueryOptions(restArgs, { key: 'uniqueOrderLineItemID', value: uniqueOrderLineItemID });
        orderLineItem = await searchOne(contractName, searchArgs, options, user);
    }
    if (!orderLineItem) {
        return undefined;
    ...
    return marshalOut({ ...orderLineItem, 
    });
}
