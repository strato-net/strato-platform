...
const contractName = 'UserMembership_2';
const contractFilename = `${util.cwd}/dapp/userMemberships/contracts/UserMembership.sol`;

/** 
 * Upload a new UserMembership 
 * @param user User token (typically an admin)
 * @param _constructorArgs Arguments of userMembership's constructor
 ...
 * 
 * As our arguments come into the userMembership contract they first pass through `marshalIn` and 
 ...
 * 
 * As our arguments come into the userMembership contract they first pass through {@link marshalIn `marshalIn`} 
...
/**
 * Bind functions relevant for userMembership to the _contract object. 
 * @param user User token
 * @param _contract Contract object from `rest.createContract()` etc.
 * @param options UserMembership deployment options (found in _/config/*.config.yaml_ via _load.config.js_)
 */
...
function bind(user, _contract, options) {
    ...
    contract.getRoles = async (args) => getRoles(user, contract, args, options);
...
/** 
 * Bind an existing UserMembership contract to a new user token. Useful for having multiple users test
 * the same contract.
 * @example <caption>Create an admin and user bound to the same new userMembership contract.</caption>
 ...
 * Update getRoles
 */
async function getRoles(admin, contract, _args, baseOptions) {
    ...
    const [userRoles] = await rest.call(admin, callArgs, options)
