 ...
 */
const migrateBatch = async (token, oldReserveAddress, newReserveAddress, escrowBatch) => {
  const callArgs = {
  ...
  if (final.status !== "Success") {
    throw new Error(`Error: migrateReserve failed for batch ${JSON.stringify(escrowBatch)}`);
  }
 ...
 * @param {Array<string>} escrowBatch - Batch of escrow addresses.
 */
const updateOldEscrowDataBatch = async (token, newReserveAddress, escrowBatch) => {
  const callArgs = {
  ...
  console.log(`Updating old escrow data for batch: ${JSON.stringify(escrowBatch)}`);
  const finalResults = await callAndWait(token, callArgs);
  const final = Array.isArray(finalResults) ? finalResults[0] : finalResults;
  if (final.status !== "Success") {
    throw new Error(`Error: updateOldEscrowData failed for batch ${JSON.stringify(escrowBatch)}`);
  }
 ...
 * @param {Array<string>} escrowBatch - Batch of escrow addresses.
 */
const updateOldEscrowBorrowDataBatch = async (token, newReserveAddress, escrowBatch) => {
  const callArgs = {
    contract: { address: newReserveAddress },
  ...
  console.log(`Updating old escrow borrow data for batch: ${JSON.stringify(escrowBatch)}`);
  const finalResults = await callAndWait(token, callArgs);
  const final = Array.isArray(finalResults) ? finalResults[0] : finalResults;
  if (final.status !== "Success") {
    throw new Error(`Error: updateOldEscrowBorrowData failed for batch ${JSON.stringify(escrowBatch)}`);
  }
...
async function main() {
  try {
    ...
    const { USERNAME, PASSWORD, OLD_RESERVE_ADDRESS, NEW_RESERVE_ADDRESS } = process.env;
    ...
    const escrowResults = await rest.search(token, { name: "BlockApps-Mercata-Escrow" }, escrowQuery);
    ...
    if (escrowAddresses.length === 0) {
      console.log(`No active escrows found for reserve ${OLD_RESERVE_ADDRESS}. Nothing to migrate.`);
      return;
    }

    console.log(`Found ${escrowAddresses.length} escrow(s) for reserve ${OLD_RESERVE_ADDRESS}.`);
    ...
    for (const batch of batches) {
      ...
      await migrateBatch(token, OLD_RESERVE_ADDRESS, NEW_RESERVE_ADDRESS, batch);
