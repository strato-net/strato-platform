...
async function main() {
  try {
    ...
    console.log(`Calling 'deactivate' on reserve at address: ${OLD_RESERVE_ADDRESS}...`);
    const callResponse = await rest.call(token, callArgs, { config, cacheNonce: true, isAsync: true });
    ...
    console.log(`Reserve at address ${OLD_RESERVE_ADDRESS} has been deactivated.`);
  } catch (error) {
