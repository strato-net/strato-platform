    ...
    return {
      ...
      status: e.response
        ...
        : e.code
          ...
          : 'NO_CONNECTION',
      message:
        'error while getting user - do they exist yet? (has their key been created yet?)',
    };
  }
}
...
const getUsernameFromDecodedToken = (decodedToken) => {
  const { tokenUsernameProperty, tokenUsernamePropertyServiceFlow } =
...
const getServiceToken = async (req = null) => {
  ...
  if (
    ...
    !expiresAt ||
    expiresAt <=
      Math.floor(Date.now() / 1000) + constants.tokenLifetimeReserveSeconds
  ) {
