...
class AuthHandler {
  ...
  static getDeployersTokenForWebhook() {
    return async function (req, res, next) {
      try {
        let globalAdminToken = await oauthHelper.getUserToken(`${process.env.GLOBAL_ADMIN_NAME}`, `${process.env.GLOBAL_ADMIN_PASSWORD}`,)
        let address
        ...
        if (globalAdminToken) {
          ...
          try {
            ...
            rest.response.status(RestStatus.BAD_REQUEST, res, 'Access token is not a valid JWT',)
            return next()
          }
          try {
            address = await rest.getKey({ username: decodedToken.preferred_username, token: globalAdminToken }, { config })
          } catch (e) {
            // user isn't created in STRATO
            if (e.response && e.response.status === RestStatus.BAD_REQUEST) {
              console.log('User not created in STRATO!')
