...
describe('Order End-To-End Tests', function () {
  ...
  it('Delete user address', async () => {
    ...
    const userAddress = createUserAddressResponse.body.data[1]
    console.log("userAddress", userAddress)
    const deleteUserAddressArgs = { address: userAddress }

    // delete
    const deleteUserAddressResponse = await post(
      Order.prefix,
      Order.deleteUserAddress,
      {address: userAddress, user: {token: seller.token, address: seller.address}},
      seller.token,
    )

    const getUserAddressResponse = await get(
      Order.prefix,
    ...
    const updatedAddress = getUserAddressResponse.body.data.filter((address) => address.address === userAddress)

    console.log("updatedAddress", updatedAddress)

    assert.equal(deleteUserAddressResponse.status, RestStatus.OK, 'should be 200');
    assert.isDefined(deleteUserAddressResponse.body, 'body should be defined')
    assert.equal(updatedAddress[0].isDeleted, true, 'address should be deleted')
  })
