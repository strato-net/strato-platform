...
describe('Inventory End-To-End Tests', function () {
  ...
  it('Get an Inventory', async () => {
    ...
    assert.equal(getInventory.body.data['productId'], createInventoryArgs['productAddress'], 'productAddress should be equal');
    assert.equal(getInventory.body.data['quantity'], createInventoryArgs['quantity'], 'quantity should be equal');
    assert.equal(getInventory.body.data['pricePerUnit'], createInventoryArgs['pricePerUnit'], 'pricePerUnit should be equal');
    assert.equal(getInventory.body.data['batchId'], createInventoryArgs['batchId'], 'batchId should be equal');
    assert.equal(getInventory.body.data['status'], createInventoryArgs['status'], 'status should be equal');
    assert.equal(getInventory.body.data['createdAt'], createInventoryArgs['createdAt'], 'createdAt should be equal');
  })
  ...
  it('Update an Inventory', async () => {
    ...
    const updateArgs = {
      ...updateInventoryArgs(productAddress, createdInventoryAddress, util.uid())
    }
  ...
  it('should not create inventories of duplicate serial numbers for same product', async () => {
    ...
    assert.isDefined(createInventoryResponse1.body.data, 'body.data should be defined')
