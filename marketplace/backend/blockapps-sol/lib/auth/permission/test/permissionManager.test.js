...
describe('PermissionManager tests', function () {
  ...
  before(async function () {
    ...
    const source = fsUtil.get(
      `${util.cwd}/${config.libPath}/auth/permission/contracts/EventLogType.sol`
    );
  ...
  it('Check permissions', async function () {
    ...
    // check
    const args = {
      address: permitArgs.address,
      permissions: permitArgs.permissions,
    };
  ...
  it('Transfer Ownership - AUTHORIZED', async function () {
    ...
    const newOwner = await createUser(
      { username: `NewOwner_${uid}`, password: '1234' },
      options
    );
    const contract = await permissionManagerJs.uploadContract(admin, master);
  ...
  it('Transfer Ownership - UNAUTHORIZED', async function () {
    ...
    // transfer ownership to attacker
    {
      const attacker = await createUser(
        { username: `Attacker_${uid}`, password: '1234' },
        options
      );
