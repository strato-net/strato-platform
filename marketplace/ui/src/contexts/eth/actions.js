  ...
  addHash: async (dispatch, payload) => {
    ...
    try {
      ...
      if (response.status === RestStatus.OK) {
        ...
        actions.setMessage(dispatch, `Successfully initiated the bridging of ${payload.amount} ${payload.tokenName.endsWith("st") ? payload.tokenName.slice(0, -2) : payload.tokenName}. to ${payload.amount} ${payload.tokenName}.`, true);
        return true;
  ...
  bridgeOut: async (dispatch, payload) => {
    ...
    try {
      ...
      if (response.status === RestStatus.OK) {
        ...
        actions.setMessage(dispatch, `Successfully initiated the bridging of ${quantityNumber} ${tokenName.endsWith("st") ? tokenName.slice(0, -2) : tokenName} to ${quantityNumber} ${tokenName}.`, true);
        return true;
      } else if (response.status === RestStatus.CONFLICT) {
        dispatch({
