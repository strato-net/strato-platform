...
const ProductDetails = ({ user, users }) => {
  ...
  const [isCalledFromInventory, setIsCalledFromInventory] = useState(false);
  const [isCalledFromWallet, setIsCalledFromWallet] = useState(false);

  useEffect(() => {
    if (state !== null && state !== undefined) {
      if (state.isCalledFromWallet) {
        setIsCalledFromWallet(true);
        setIsCalledFromInventory(false);
      } else if (state.isCalledFromInventory) {
        setIsCalledFromInventory(true);
        setIsCalledFromWallet(false);
      }
  ...
  useEffect(() => {
    if (isCalledFromInventory) {
      setId(routeMatch1?.params?.id);
    } else if (isCalledFromWallet) {
      setId(routeMatch2?.params?.id);
    } else {
                    ...
                    {details?.price ? (
                      <>
                        ...
                        <span className="font-normal text-xs mr-2 text-primary">
                          <b>
                            ({(details?.price * STRATS_CONVERSION).toFixed(0)}{" "}
                            STRATS)
                          </b>
                        </span>
                      </>
