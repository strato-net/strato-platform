...
const NestedComponent = () => {
            ...
            <>
                <Row className="w-full flex items-center my-2">
                    <div className="flex flex-auto justify-center items-center gap-2 text-center">
                        <div className="cursor-pointer" style={{ marginLeft: marginVal }} onClick={() => callSubMaterial(elem)}>
                            {expandedKeys.includes(elem.key) ? <MinusCircleOutlined /> : <PlusCircleOutlined />}
                        </div>
                        <p className="text-primaryB" >{decodeURIComponent(elem.rawMaterialProductName)}</p>


                    </div>
                    <div className="flex-auto text-center" style={{ marginLeft: marginVal }}>
                        <Text className="text-primaryB">{elem.rawMaterialSerialNumber}</Text>
                    </div>
                </Row>
                ...
                {elem.children && expandedKeys.includes(elem.key) ?
                    elem.children.length === 0 ?
                        <>
                            <Row className="justify-center"><Text className="text-primaryC">No raw materials found</Text></Row>
                            <Divider className="mx-0 my-3" />
                        </> :
                        elem.children.map(childElem => {
                            let currentLevel = level + 1;
                            return <ExpandedView elem={childElem} level={currentLevel} key={childElem.key} />
                        }) : null}
            </>
        );
    }
    ...
    return (
        <>
            ...
            <Divider className="mx-0 my-3" />
            {
                actualRawMaterials.map(elem => <ExpandedView elem={elem} level={0} key={elem.key} />)
            }
