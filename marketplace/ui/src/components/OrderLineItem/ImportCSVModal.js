...
const ImportCSVModal = ({
  ...
  const onFileLoad = (_dropFiles, acceptedFiles, _rejectedFiles) => {
    ...
    reader.onload = evt => {
      ...
      if (!contents.data[0]['quantity']) {
        actions.setMessage(dispatch, "Missing required column 'quantity'");
        return;
      }

      
      if (!contents.data[0]['pricePerUnit']) {
        actions.setMessage(dispatch, "Missing required column 'pricePerUnit'");
        return;
      }
