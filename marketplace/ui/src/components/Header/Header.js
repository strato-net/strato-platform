...
const HeaderComponent = ({ user, loginUrl }) => {
  ...
  const storedData = useMemo(() => {
    return window.localStorage.getItem("cartList") == null
      ? []
  ...
  useEffect(() => {
    ...
    if (
      ...
      pathName.includes("sold-orders") ||
      pathName.includes("bought-orders")
    ) {
      setSelectedTab("1");
        ...
        },
        {
          ...
          label: (
            <div
              type="text"
              id="logout"
              className="w-full text-secondryB text-sm !hover:bg-success"
              onClick={logout}
            >
              Logout
            </div>
  ...
  useEffect(() => {
    ...
    if (user != null) {
      if (user.commonName.split(" ").length > 1) {
        temp =
          user.commonName.split(" ")[0].substring(0, 1) +
          user.commonName.split(" ")[1].substring(0, 1);
      } else {
  ...
  return (
    <Header className="!bg-primary flex">
      ...
      <Space size="large">
        {!(roleIndex === undefined || roleIndex === 1) && (
          <Button className="border-0 mt-5" onClick={() => setOpen(true)}>
            {sell}
