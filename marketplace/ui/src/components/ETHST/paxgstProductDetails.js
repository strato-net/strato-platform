...
const PaxgProductDetails = ({ user, users }) => {
    ...
    const fetchBalance = async () => {
      if (address) {
        const paxgAddress = fileServerUrl.includes("test")
          ? "0x29f2D40B0605204364af54EC677bD022dA425d03" // PAXG testnet contract
          : "0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599"; // PAXG mainnet contract
  
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        const signer = provider.getSigner();
        setSigner(signer);
  
        // Create PAXG contract instance
        const paxgContract = new ethers.Contract(paxgAddress, ERC20_ABI, provider);
  
        try {
          // Get PAXG balance
          const paxgBalance = await paxgContract.balanceOf(address);
  
          // PAXG has 8 decimals (like BTC), format accordingly
          const formattedBalance = ethers.utils.formatUnits(paxgBalance, 8); // 8 decimals for PAXG
          setPaxgBalance(formattedBalance); // Set PAXG balance
        } catch (error) {
          console.error("Failed to fetch PAXG balance:", error);
        }
      }
    };
...
export default PaxgProductDetails;
