...
const UsdcProductDetails = ({ user, users }) => {
    ...
    const fetchBalance = async () => {
      if (address) {
        const usdcAddress = fileServerUrl.includes("test")
          ? "0x29f2D40B0605204364af54EC677bD022dA425d03" // USDC testnet contract
          : "0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599"; // USDC mainnet contract
        ...
        try {
          ...
          // USDC has 8 decimals (like BTC), format accordingly
          const formattedBalance = ethers.utils.formatUnits(usdcBalance, 8); // 8 decimals for USDC
          setUsdcBalance(formattedBalance); // Set USDC balance
        } catch (error) {
...
export default UsdcProductDetails;
