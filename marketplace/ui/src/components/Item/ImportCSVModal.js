...
const ImportCSVModal = ({
  ...
  const onFileLoad = (_dropFiles, acceptedFiles, _rejectedFiles) => {
    ...
    reader.onload = evt => {
      ...
      if (!contents.data[0]['serialNumber']) {
        actions.setMessage(dispatch, "Missing required column 'serialNumber'");
        return;
      }
      ...
      if (!contents.data[0]['comment']) {
        actions.setMessage(dispatch, "Missing required column 'comment'");
        return;
      }
