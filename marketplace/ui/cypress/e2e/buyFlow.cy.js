...
describe('Create a new Asset', () => {
  ...
  it('It should add a new product', () => {
    ...
    cy.get('#artist')
    cy.get('#artist').type(productName).should('have.value', productName)
  ...
  it.only('It should buy a product', () => {
    cy.visit('/');
    ...
    cy.get('#price').then(productPrice => {
      ...
      cy.get('#totalPrice').invoke('text').then(totalPriceText => {
        ...
        cy.wait(15000);
        cy.get('#address-list-block').should('exist').then($element => {
          if ($element.hasClass('address-list')) {
