...
describe("Renders Inventory Page", () => {
  ...
  it("it should render inventory list page", () => {
    ...
    cy.request({
      ...
      if (body.data.length !== 0) {
        ...
        inventoryList.get("#0").contains(decodeURIComponent(details.manufacturer)).should("be.visible");
        inventoryList.get("#0").contains("Price Per Unit").should("be.visible");
        inventoryList.get("#0").contains(details.pricePerUnit).should("be.visible");
        inventoryList.get("#0").contains("Least Sellable Unit").should("be.visible");
        inventoryList.get("#0").contains(details.leastSellableUnit).should("be.visible");
        inventoryList.get("#0").contains("Batch ID").should("be.visible");
        inventoryList.get("#0").contains(details.batchId).should("be.visible");
        inventoryList.get("#0").contains("Remaining Quantity").should("be.visible");
        inventoryList.get("#0").contains(details.availableQuantity).should("be.visible");
        inventoryList.get("#0").contains("Serial Numbers").should("be.visible");
        inventoryList.get("#0").contains("View").should("be.visible");
        inventoryList.get("#0").contains(INVENTORY_STATUS[details.status]).should("be.visible");
        const status = details.isActive ? "Active" : "Inactive";
        inventoryList.get("#0").contains(status).should("be.visible");
        inventoryList.get("#0").get("button").contains("Preview").should("be.visible");
        ...
        inventoryList.get("#0").get(".ant-popover-open").eq(0).should("be.visible");
  ...
  it("it should preview an inventory", () => {
    ...
    cy.request({
      ...
      cy.get("#details").contains(decodeURIComponent(inventory.name)).should("be.visible");
