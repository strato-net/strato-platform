...
escapeList = B.pack . go . B.unpack
  where go (0x22:ts) = 0x5c:0x22:go ts
...
main = do
  let n = 100
  let escapes = bgroup "Escaping and Unescaping"
  ...
  let parses = bgroup "Parsing"
             [ bench "parse nothing" $ nf parsePath ""
             , bench "parse field" $ nf parsePath ".field"
             , bench "parse one long field" $ nf parsePath
               $ "." <> B.replicate 100 0x45
             , bench "parse one long map index" $ nf parsePath
