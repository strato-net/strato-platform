...
if [ "$REPO_NAME" != "strato-platform" ]; then
    echo "ERROR: This script must be run from within the strato-platform repository"
...
if ! git rev-parse --is-inside-work-tree &> /dev/null; then
    ...
    echo "Please run this script from within the strato-platform git repository"
    exit 1
fi

# Warning if not run from scripts directory
EXPECTED_SCRIPT_DIR="${PROJECT_DIR}/scripts"
if [ "$SCRIPT_DIR" != "$EXPECTED_SCRIPT_DIR" ]; then
    echo "WARNING: This script is not in the expected location"
    echo "Expected: ${EXPECTED_SCRIPT_DIR}"
    echo "Actual: ${SCRIPT_DIR}"
    echo "The script will still run, but it's recommended to run it from scripts/"
fi
...
log_info "Checking for timeout..."
if ! command -v timeout &> /dev/null; then
    log_error "timeout not found"
    exit 1
fi
log_success "timeout found"
