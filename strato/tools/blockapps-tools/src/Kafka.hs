  ...
  if done
    then return Nothing
      ...
      when (bytesRead < headerLen)
        . die
        $ printf "decoding failure: header too short: %d < %d" bytesRead headerLen
      wl <- fromBE <$> peek lbuf :: IO Word32
      msg <- B.hGet h (fromIntegral wl)
      when (B.length msg < fromIntegral wl)
        . die
        $ printf "decoding failure: payload too short: %d < %d" (B.length msg) wl
      return $ Just msg
...
loadKafka topic file = do
  withBinaryFile file ReadMode $ \h -> do
    hSetBuffering h . BlockBuffering . Just $ 1024 * 1024
    res <-
      runKafkaConfigured "queryStrato" . runConduit $
        streamFile h
          .| bundleMessages
...
verifyKafkaFile file = do
  withBinaryFile file ReadMode $ \h -> do
    hSetBuffering h . BlockBuffering . Just $ 1024 * 1024
    (msgsRead, bytesRead) :: (Int, Int) <-
      flip execStateT (0, 0) . runConduit $
        streamFile h
          .| iterMC (\_ -> _1 += 1)
