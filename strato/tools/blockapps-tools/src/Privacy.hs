...
getSomeKeysInNamespace ::
  ...
  Int ->
  m [NSKey a]
getSomeKeysInNamespace p' start' count'
  | start' <= 0 = return []
  | count' <= 0 = return []
  | otherwise = do
    db <- access Proxy
  ...
  where
    getKeysInNamespace' ::
      ...
      Collector (NSKey a) ->
      m [NSKey a]
    getKeysInNamespace' p i c
      | keyCount c <= 0 = return $ keys c
      | otherwise = do
...
handlePut ::
  (HasNamespace a, Binary a) => -- Read (NSKey a), FromJSON (NSKey a), Read a, FromJSON a)
  Proxy a ->
  NSKey a ->
  a ->
  IO ()
...
handleDelete ::
  (HasNamespace a) => --, Read (NSKey a), FromJSON (NSKey a))
  Proxy a ->
  NSKey a ->
  IO String
...
getPrivacy registry key js = case registry of
  ...
  where
    jsEncode _ = T.unpack . decodeUtf8 . BL.toStrict . Ae.encode
