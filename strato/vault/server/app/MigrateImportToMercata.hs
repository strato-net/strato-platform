...
main :: IO () 
main = do
  _ <- $initHFlags "migrate-import-to-mercata"
  putStrLn "Usage: migrate-export-to-mercata --pw=<vault_password>  pwOld=<vault_password>"
  ...
  let reencrypt :: B.ByteString -> Int32 -> B.ByteString -> SB.Nonce -> B.ByteString -> Maybe (B.ByteString, Int32)
      reencrypt _ i _ nonce encKey = do
          let newEncKey =  VC.encrypt pwKey nonce encKey
          pure (newEncKey, i)
