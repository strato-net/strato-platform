    ...
    describe "indexEventToTxrResults properly indexes events into txr results" $ do
        it "Index EventDBEntry for MemberAdded" $
            let addr = fromInteger 0x3023
                ...
                event = EventDB (read "0xdeadbeef") (Just chainId) "MemberAdded" [show addr, showEnode enode]
        ...
        it "Index EventDBEntry for MemberRemoved" $
            let addr = fromInteger 0x1011
                ...
                event = EventDB (read "0xdeadbeef") (Just chainId) "MemberRemoved" [show addr]
            in indexEventToTxrResults (EventDBEntry event)
        ...
        it "Index EventDBEntry for CertificateRegistered" $
            let certString = unlines
                ...
                event = EventDB (read "0xdeadbeef") Nothing "CertificateRegistered" [certString]
            ...
            in indexEventToTxrResults (EventDBEntry event)
                `shouldBe` [PutEventDB event, RegisterCertificate $ Right ((read "0xdeadbeef"), addr, X509CertInfoState{userAddress=addr, certificate=parsedCert, isValid=True, children=[]})]
        it "Index EventDB for CertificateRevoked" $
            let userAddr = fromInteger 0x489384
                event = EventDB (read "0xdeadbeef") Nothing "CertificateRevoked" [show userAddr]
            in indexEventToTxrResults (EventDBEntry event)
                `shouldBe` [PutEventDB event, CertificateRevoked . Right $ ((read "0xdeadbeef"), userAddr)]
        it "Index EventDB for CertificateRegistryInitialized" $
            let event = EventDB (read "0xdeadbeef") Nothing "CertificateRegistryInitialized" []
            in indexEventToTxrResults (EventDBEntry event)
                `shouldBe` [PutEventDB event, CertificateRegistryInitialized . Right $ ((read "0xdeadbeef"))]
        it "Index EventDBEntry for non-special event" $
            let chainId = fromInteger 0x480244
                event = EventDB (read "0xdeadbeef") (Just chainId) "NotSpecial" ["48193"]
            in indexEventToTxrResults (EventDBEntry event)
