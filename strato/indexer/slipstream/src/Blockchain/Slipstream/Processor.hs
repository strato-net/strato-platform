...
processedContractToProcessedCollectionRows state mapAndArrayNames row abiid cregator =
  let valueCollectionsMap = Map.filter (\value -> case value of 
   ...
   in if null valueCollectionsMap  
        then []
        else 
          let processRecord :: (Text, Either (Map.Map SimpleValue Value) (Either [Value] (I.IntMap Value))) -> [ProcessedCollectionRow]
           ...
           in concatMap processRecord recordVMs
        ...
        case actionStorage row of
          ...
          Action.SolidVMDiff {} -> do
            ...
            let stateDiff = rowToCollections row
                parents' = map (\(_,_,_,p ,_)-> p) abstractColumns'
                abstractColumns = map (\(a,b,c,_,e) -> (a,b,c,e)) abstractColumns'
