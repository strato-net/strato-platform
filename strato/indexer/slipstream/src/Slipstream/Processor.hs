    ...
    mkAggregateEvent a e =
      AggregateEvent
        { eventBlockHash = Action._blockHash a,
          ...
          eventAbstracts = Map.map (\(x, _) -> x) $ maybe Map.empty Action._actionDataAbstracts . OMap.lookup (evContractAccount e) $ Action._actionData a,
          eventEvent = e
        }
...
processTheMessages env conn messages = do
  ...
  let changes = parseActions messages
      ...
      creates = [(cc, cp, cn, a, hl, abs', rm) | CodeCollectionAdded cc cp cn a hl abs' rm <- messages]
      -- delegates = [d | DelegatecallMade d <- messages]
      transactionResults = [tr | NewTransactionResult tr <- messages]

  fkeys' <- forM creates $ \(cc, cp, cn, _, hl, abstracts', _) -> do
        ...
        deferredForeignKeys <- fmap concat $
          forM (Map.toList $ cc ^. contracts) $ \(_, c) -> do
            ...
            deferredForeignKeys <- case (_contractType c) of
              AbstractType -> do
                _ <- outputData conn $ createExpandAbstractTable g c nameParts (Map.map (\(x, _) -> x) abstracts') cc
