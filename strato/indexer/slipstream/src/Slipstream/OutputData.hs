...
module Slipstream.OutputData
  ( outputData,
    ...
    keyColumnNamesWithoutType,
    getPreviousBlockNumberSql,
  )
where
...
collectionHistoryTableName creator a n m =
  let (c', a', n') = constructTableNameParameters creator a n
   in CollectionHistoryTableName c' a' n' m
...
compareCollectionRows x y =
  collectionDataKeys x == collectionDataKeys y
    && creator x == creator y
...
instance Ord ForeignKeyInfo where
  compare x y =
    compare
      (tableName x, columnNames x, foreignTableName x, foreignColumnNames x)
...
createForeignIndexesForJoins foreignKey = do
  let srcTable = textToDoubleQuoteText $ tableNameToTextPostgres (tableName foreignKey)
      ...
      logMessage =
        "createForeignIndexesForJoins srcTable: "
          <> (T.pack $ show $ tableName foreignKey)
          <> ", targetTable: "
          <> (T.pack $ show $ foreignTableName foreignKey)
...
historyValidToColumnQuery :: [Text]
historyValidToColumnQuery = [historyValidToColumn <> " text"]
...
keyColumnNamesToCondition :: [Text] -> Text
keyColumnNamesToCondition keyNames =
  T.intercalate " AND " $ map (\(k) -> k <> " = NEW." <> k) keyNames

getPreviousBlockNumberSql :: Text
getPreviousBlockNumberSql =
  T.pack " CAST( CAST (NEW.block_number AS INTEGER) - 1 AS TEXT) "
...
createCollectionHistoryTableQuery creator a n collectionName keyTypes valueType =
  let collectionHistoryTableName' = collectionHistoryTableName creator a n collectionName
      collectionTableName' = collectionTableName creator a n collectionName
      keyNames = keyColumnNames keyTypes
      keyNamesWithoutType' = keyColumnNamesWithoutType keyTypes
      -- normalTableName = bool (indexTableName creator a n) (abstractTableName creator a n) isAbstract
      triggerFunctionName = "\"" <> "insert_or_update_" <> tableNameToText collectionHistoryTableName' <> "_history_table" <> "\""
   in T.concat
        [ "CREATE TABLE IF NOT EXISTS ",
          ...
          "BEGIN\n",
          "    RAISE NOTICE 'Trigger fired for % on table ",
          tableNameToText collectionTableName',
          ": %', TG_OP, NEW.address;\n",
          "    IF TG_OP = 'INSERT' THEN\n",
          "        RAISE NOTICE 'Inserting into history table ",
          ...
          maximumBlockNumber,
          " AND ",
          keyColumnNamesToCondition keyNamesWithoutType',
          ";\n ",
          -- insert part
          "        INSERT INTO ",
          tableNameToDoubleQuoteText collectionHistoryTableName',
          ...
          "\"",
          "\nAFTER INSERT ON ",
          tableNameToDoubleQuoteText collectionTableName',
          "\nFOR EACH ROW EXECUTE PROCEDURE ",
          triggerFunctionName,
          "();\n\n",
          -- Create trigger for update operations
...
insertAbstractTableQuery cs =
  concat $
    let cs' =
          ( \(c@E.ProcessedContract {contractData = contractData}, fkeys, ab, abColumns) ->
              ((c, Map.mapMaybe valueToSQLTextFilterContract $ contractData), (ab, abColumns, fkeys))
          )
            <$> cs
          ...
          contracts@(((_, _), (abTableName, abColumns, fkeys)) : _) ->
            let fkeyColumns = [k | k <- fkeys, k `elem` abColumns]
                ...
                vals = flip map contracts $ \((_, contractColumns), _) ->
                  let contractValEntries = Map.toList contractColumns
...
updateFkeysQueryArray rows = concatMap createUpdateQuery rows
  where
    ...
    createUpdateQuery c =
      let tableName = case eventInfo c of
...
createEventTable (creator, a, n) evName ev cc = do
  ...
  let (crtr, app, cname) = constructTableNameParameters creator a n
      ...
      indexedFields =
        map (wrapDoubleQuotes . escapeQuotes . fst)
          . filter snd
  ...
  case li of
    ...
    (arrayName, arrayStr) : _ ->
      let elements = fromMaybe [] (Aeson.decode (BL.fromStrict $ TE.encodeUtf8 $ T.pack arrayStr) :: Maybe [String])
       in ( arrayName,
            zip
              (map (SimpleValue . ValueString . T.pack . show) [0 :: Int ..])
...
insertEventTables processedEventArrays processedEventsWithoutArrays = do
  ...
  when (not (null processedEventArrays)) $
    yieldMany $
      insertEventArrayTableQuery processedEventArrays
