        ...
        contracts@(((x, _), (abTableName, abColumns)):_) ->
          let contractTableName = indexTableName
              ...
              vals = flip map contracts $ \((row, contractColumns),_) ->
                wrapAndEscape $ map (wrapSingleQuotes . ($ row)) baseVals ++ [wrapSingleQuotes $ T.pack $ show $ Aeson.encode $ MapWrapper $ aesonHelper $ Map.filterWithKey (\k _ -> k `notElem` abColumns) contractColumns] ++ [wrapSingleQuotes (tableNameToText contractTableName)] ++ (map snd $ Map.toList (Map.filterWithKey (\k _ -> k `elem` abColumns) contractColumns))
              inserts = csv vals
           in (:[]) $ T.concat
                [ "INSERT INTO "
                , abTableName
