...
def wait_for_slipstream_to_sync(node1_url, node2_url, headers1, headers2, attempts, sleep_time):
    ...
    while True:
        attempt += 1
        try:
            response1 = requests.get(node1_url + "/cirrus/search/BlockApps-Mercata-Asset.ItemTransfers", headers=headers1, params={'order':'block_timestamp.desc', 'limit':1})
            response2 = requests.get(node2_url + "/cirrus/search/BlockApps-Mercata-Asset.ItemTransfers", headers=headers2, params={'order':'block_timestamp.desc', 'limit':1})
