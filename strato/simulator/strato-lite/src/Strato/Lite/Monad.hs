...
instance MonadIO m => A.Selectable Address X509CertInfoState (MonadTest m) where
  select _ addr = use $ x509Map . at addr

instance MonadIO m => (Address `A.Alters` X509CertInfoState) (MonadTest m) where
  lookup _ k   = use $ x509Map . at k
  insert _ k v = x509Map . at k ?= v
  delete _ k   = x509Map %= sans k
...
instance (Address `A.Alters` X509CertInfoState) m => (Address `A.Alters` X509CertInfoState) (MonadP2PTest m) where
  lookup p k   = lift $ A.lookup p k
  insert p k v = lift $ A.insert p k v
  delete p k   = lift $ A.delete p k 

instance Mod.Accessible CertificateRegistryInitialized m => Mod.Accessible CertificateRegistryInitialized (MonadP2PTest m) where
  access p = Mod.access p
