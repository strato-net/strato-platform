...
call' from to' fnCalltype functionName valList = profileRunCodeChild
  ...
  f <- (nullifyRefs =<<) <$> case (lookupFunction functionName', fnCalltype) of
      ...
      (Just theFunction, CC.DefaultCall) -> do
        ...
        let canonicalBlockRewardCall =
              contract ^. CC.contractName == "HeliumFeeRouter"
                && functionName' == "payBlockRewards"
                && null valList
                && null (CC._funcOverload theFunction)
            runDefaultInterpreted = do
        ...
        if canonicalBlockRewardCall
          then validateFunctionArguments cc contract theFunction valList >>= \case
            Just (theFunction', []) ->
              pure . bool id (pushSender from) shouldPushSender $
                runTheCallValidated AlreadyValidated storageAddress codeAddress contract functionName' hsh cc theFunction' [] currentReadOnly False
            _ -> runDefaultInterpreted
          else runDefaultInterpreted
