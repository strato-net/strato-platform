...
callWrapper' from to' mLogicAddress mContract functionName isRCC argExps = do
  ...
  let (functionName', argsParsed) =
        if isDelegateCall
          then
            ( case runParser parseDelegateCallArgs (ParserState "" "" M.empty) "" functionName of
  ...
  (!f, !args) <-
    case M.lookup functionName' functionsIncludingConstructor of
      ...
      Just theFunction | (isDelegateCall == True) -> do
        let mtheFunction' = do
              -- find the func that matches the arguements the user gave, if not found return Nothing and then call fallback
