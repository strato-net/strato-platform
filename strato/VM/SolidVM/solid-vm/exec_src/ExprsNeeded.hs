...
main = do
  ...
  File parsedFile <- either (die . show) return $ runParser solidityFile (ParserState "" "") "" contents
  let pragmas' = \case
  ...
  let vmVersion' = if (Just ("solidvm","3.3")) `elem` (pragmas' <$> parsedFile) then "svm3.3" else (if (Just ("solidvm","3.2")) `elem` (pragmas' <$> parsedFile) then "svm3.2" else (if (Just ("solidvm","3.0")) `elem` (pragmas' <$> parsedFile) then "svm3.0" else ""))
      namedContracts = [(textToLabel name, either (throw . fst) id $ xabiToContract (textToLabel name) (map textToLabel parents') vmVersion' xabi) | NamedXabi name (xabi, parents') <- parsedFile]
