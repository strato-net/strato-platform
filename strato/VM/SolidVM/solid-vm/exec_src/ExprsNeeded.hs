...
main = do
  ...
  let pragmas' = \case
        ...
        _ -> Nothing
  let namedContracts = [(textToLabel name, either (throw . fst) id $ xabiToContract (textToLabel name) (map textToLabel parents') vmVersion' M.empty xabi) | NamedXabi name (xabi, parents') <- parsedFile]
