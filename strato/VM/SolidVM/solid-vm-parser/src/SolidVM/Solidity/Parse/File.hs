...
solidityFile = do
  whiteSpace
  units <- many (solidityPragma <|> solidityImport <|> solidityFreeFunction <|> solidityContract <|> solidityFLConstant <|> solidityFLStruct <|> solidityFLEnum <|> solidityUsingDirective)
  eof
  return . File $ units
