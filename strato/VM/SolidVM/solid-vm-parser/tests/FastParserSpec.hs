...
import SolidVM.Solidity.Parse.File (decideVersion)
import SolidVM.Solidity.Parse.Fast.Monad
import SolidVM.Solidity.Parse.Fast.Parser
...
-- | Runs a rule over a string with the given state.
runWith :: ParserState -> P a -> String -> Either ParseError a
runWith st p = runRule p st "" . T.pack

run :: P a -> String -> Either ParseError a
run = runWith initialParserState
...
spec = do
  describe "String lexing" $ do
    let parseStr = run (stringLiteral <* eof)
        cases =
          [ ([r|"ok"|], "ok"),
            ([r|"ok" |], "ok")
          ]
  ...
  describe "Expression parsing" $ do
    let parseExpr = fmap (fmap (const ())) . run (expression <* eof)
        cases =
  ...
  describe "Operator precedence" $ do
    let parseExpr = fmap (fmap (const ())) . run (expression <* eof)
        ...
        parseLegacy = fmap (fmap (const ())) . runWith legacyState (expression <* eof)
        v = Variable ()
        bin = Binary ()
        cases =
  ...
  describe "Arg literal parsing (parseArg)" $ do
    let parseA = fmap (fmap (const ())) . run (literal <* eof)
        cases =
  ...
  describe "Files" $ do
    let parseFile = parseSolidity initialParserState ""
    it "rejects a pragma without an identifier" $
      parseFile "pragma;" `shouldSatisfy` isLeft
    it "accepts a pragma without contents" $
      parseFile "pragma typecheck;" `shouldSatisfy` isRight
    it "rejects a pragma without a ;" $
      parseFile "pragma solc 0.4.8" `shouldSatisfy` isLeft
    let findVer = fmap decideVersion . parseFile
    it "reads the solidity version from the pragma" $ do
