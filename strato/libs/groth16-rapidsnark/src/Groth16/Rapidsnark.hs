...
-- | Groth16 proof generation via rapidsnark CLI
--
-- This module wraps the rapidsnark C++ prover for fast Groth16 proof generation.
-- Uses snarkjs for witness calculation (rapidsnark only does proving).
-- Requires both rapidsnark and snarkjs to be installed.
--
module Groth16.Rapidsnark
  ( -- * Types
    Config(..)
  , Proof(..)
  , G1Point(..)
...
-- | Configuration for rapidsnark prover
data Config = Config
  { cfgCircuitWasm   :: !FilePath  -- ^ Path to circuit .wasm file (for witness gen)
  , cfgProvingKey    :: !FilePath  -- ^ Path to .zkey proving key
  , cfgRapidsnarkPath :: !FilePath -- ^ Path to rapidsnark executable
  , cfgSnarkjsPath   :: !FilePath  -- ^ Path to snarkjs executable (for witness gen)
  } deriving (Show, Eq)

-- | Generate a Groth16 proof using rapidsnark
--
-- Takes circuit inputs as JSON ByteString, returns proof or error.
-- Uses snarkjs for witness calculation, rapidsnark for proof generation.
generateProof :: Config -> LBS.ByteString -> IO (Either Text Proof)
generateProof Config{..} inputJson = do
  withSystemTempDirectory "groth16_rapidsnark" $ \tmpDir -> do
    ...
    -- Step 1: Generate witness using snarkjs (rapidsnark doesn't do this)
    ...
    case exitCode1 of
      ...
      ExitSuccess -> do
        -- Step 2: Generate proof using rapidsnark
        let proveArgs = [cfgProvingKey, witnessFile, proofFile, publicFile]
        (exitCode2, _, stderr2) <- readProcessWithExitCode cfgRapidsnarkPath proveArgs ""
        case exitCode2 of
          ExitFailure code ->
            return $ Left $ "Rapidsnark proof generation failed (exit " <> T.pack (show code) <> "): " 
                         <> T.pack stderr2
          ExitSuccess -> do
            -- Parse the proof JSON (same format as snarkjs)
            proofJson <- LBS.readFile proofFile
            return $ parseProofJSON proofJson

-- | Parse proof from JSON output
