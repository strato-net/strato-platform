...
toMethod name f params =
  let f' args = A.toJSON `liftM` _apply f params args
   in Method name f'
...
call ::
  ...
  -- | JSON-RPC request.
  B.ByteString ->
  -- | The response wrapped in 'Just', or
  ...
  where
    ...
    parseVal val =
      case val of
        ...
        A.Array vec
          | V.null vec -> throwInvalidRpc "Empty batch request"
...
singleCall methods val = case parsed of
  ...
  Right (Request name args i) ->
    ...
    where
      method = lookupMethod name methods
  where
    parsed = runIdentity $ runExceptT $ parseValue val
...
lookupMethod name = maybe notFound return . H.lookup name
  where
    notFound = throwError $ rpcError (-32601) $ "Method not found: " `append` name
...
batchCall strategy methods vals = (noNull . catMaybes) `liftM` results
  where
    results = strategy $ map (singleCall methods) vals
