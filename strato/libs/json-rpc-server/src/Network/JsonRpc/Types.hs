...
parseArg name val = case A.fromJSON val of
  ...
  where
    argTypeError = rpcErrorWithData (-32602) $ "Wrong type for argument: " <> name
...
instance A.FromJSON Request where
  parseJSON (A.Object x) =
    (checkVersion =<< x .:? versionKey .!= jsonRpcVersion)
      *> ( Request
             <$> x .: "method"
    ...
    where
      parseParams (A.Object obj) = return $ Left obj
      parseParams (A.Array ar) = return $ Right ar
      parseParams _ = empty
      checkVersion ver =
        when (ver /= jsonRpcVersion) $
