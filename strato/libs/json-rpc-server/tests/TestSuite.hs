...
testBatch = sortBy (compare `on` rspToIdString) <$> response @?= Just expected
  where
    expected = [nullIdErrRsp (-32600), rsp i1 2, rsp i2 4]
      where
        rsp i x = defaultRsp `id'` Just i `result` A.Number x
    ...
    requests = [rq (Just i1) 10 8, rq (Just i2) 24 20, rq Nothing 15 1, defaultRq `version` Just (A.String "abc")]
      where
        rq i x y = defaultRq `id'` i `params` toArgs x y
...
testBatchNotifications = runState response 0 @?= (Nothing, 10)
  where
    response = S.call [incrementStateMethod] $ A.encode rq
...
testAllowMissingVersion = callSubtractMethods requestNoVersion @?= (Just $ defaultRsp `result` A.Number 1)
  where
    requestNoVersion = defaultRq `version` Nothing `params` Just (A.object ["x" .= A.Number 1])

incrementStateMethod :: S.Method (State Int)
incrementStateMethod = S.toMethod "increment" f ()
  where
    f :: S.RpcResult (State Int) ()
    f = lift $ modify (+ 1)
...
callSubtractMethods req =
  let methods :: [S.Method Identity]
...
callGetTimeMethod req =
  let rsp = S.call [getTimeMethod] $ A.encode req
   in (A.decode =<<) <$> rsp
...
flippedSubtractMethod = S.toMethod "flipped subtract" (flip subtract) ps
  where
    ps = S.Optional "y" (-1000) :+: S.Required "x" :+: ()
...
getTimeMethod = S.toMethod "get_time_seconds" getTestTime ()
  where
    getTestTime :: S.RpcResult IO Integer
