...
foreign import ccall unsafe "lookup3.h _jenkins_hashword2"
  hashWord2 ::
    Ptr Word32 -> CSize -> Ptr Word32 -> Ptr Word32 -> IO ()

foreign import ccall unsafe "lookup3.h _jenkins_hashlittle"
  hashLittle ::
    Ptr a -> CSize -> Word32 -> IO Word32

foreign import ccall unsafe "lookup3.h _jenkins_hashlittle2"
  hashLittle2 ::
...
rechunk s
  | LB.null s = []
  | otherwise =
    let (pre, suf) = LB.splitAt chunkSize s
...
instance
  (Hashable a, Hashable b, Hashable c, Hashable d, Hashable e) =>
  Hashable (a, b, c, d, e)
  where
...
foreign import ccall unsafe "lookup3.h _jenkins_little2_begin"
  c_begin ::
    Ptr Word32 -> Ptr Word32 -> HashState -> IO ()

foreign import ccall unsafe "lookup3.h _jenkins_little2_frag"
  c_frag ::
    Ptr a -> CSize -> HashState -> CSize -> IO CSize

foreign import ccall unsafe "lookup3.h _jenkins_little2_step"
  c_step ::
    Ptr a -> CSize -> HashState -> IO CSize

foreign import ccall unsafe "lookup3.h _jenkins_little2_end"
  c_end ::
    CInt -> Ptr Word32 -> Ptr Word32 -> HashState -> IO ()

unsafeAdjustCStringLen ::
  SB.ByteString ->
  Int ->
  (CString -> Int -> IO a) ->
  IO a
