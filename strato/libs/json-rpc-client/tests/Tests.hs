...
import Network.JsonRpc.Server (call, callWithBatchStrategy, rpcError)
...
subtractMethod = toServerMethod subtractSig f
  where
    f :: Int -> Int -> RpcResult RequestCount Int
    f x y = (x - y) <$ modify (+ 1)

divideMethod = toServerMethod divideSig f
  where
    f :: Rational -> Rational -> RpcResult RequestCount Rational
    f x y = do
