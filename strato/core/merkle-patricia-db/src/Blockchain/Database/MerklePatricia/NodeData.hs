...
proveNodeData :: (StateRoot `A.Alters` NodeData) m => KeyRange -> (Key, StateRoot) -> m (NodeDataProof (Either (Fix NodeDataProof) (Key, StateRoot)))
...
proveMP :: (StateRoot `A.Alters` NodeData) m => KeyRange -> StateRoot -> m (Fix NodeDataProof)
