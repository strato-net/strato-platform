...
type HasFullPrivacy m =
  ( HasPrivacyRegistries m,
    HasPrivateHashDB m
  )
    ...
    cHashes' <- fmap catMaybes . forM cHashes $ \cHash -> do
      ...
      case didInsert of
        ...
        SeenPreviouslyOnUnknownChain bi -> do
          $logErrorS "insertPrivateHash" . T.pack $
            concat
              [ "Chain hash ",
                format cHash,
...
insertChainHash obi cHash chainId =
  lookup Proxy cHash >>= \case
      ...
      if S.null btr
        then return []
          ...
          if Just b0 == mPreviousBlock
            then do
              $logWarnS "Privacy/runBlocks" . T.pack $
                concat
    ...
    go seen descendent =
      if descendent `S.member` seen
        then pure (True, seen)
          ...
          lookup (Proxy @ChainIdEntry) descendent >>= \case
...
hydratePrivateHashes chainF b = do
              ...
              Just cId -> do
                ...
                if isDiscluded
                  then do
                    ...
                    lookup (Proxy @ChainIdEntry) cId >>= \case
                      Nothing -> do
  ...
  txs'' <- for (zip txs txs') $ \(tx, tx') -> bool tx' tx <$> discluded newDiscludes (txChainId =<< otPrivatePayload tx')
...
insertNewChainInfo chainId cInfo = do
  ...
  insertChainHash bInfo cHash chainId >>= \case
    ...
    AlreadyExistsOnSameChain -> do
      ...
      return []
    SeenPreviouslyOnUnknownChain bi -> do
      $logErrorS "insertNewChainInfo" . T.pack $
        concat
          [ "Initial chain hash of chain ID ",
            formatChainId $ Just chainId,
