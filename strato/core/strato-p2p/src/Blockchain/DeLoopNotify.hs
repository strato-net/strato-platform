...
deLoopSource commHost commPort peerIP = labelTheThread "deloop source" $ forever $ do
  resolveIPOrHost peerIP >>= \case
    ...
    Right resolvedPeerIPs ->
      (liftIO (getPeersIO commHost commPort)) >>= \case
  ...
  where
    resolveQuietly :: (MonadIO m, MonadLogger m) => RPCPeer -> m [String]
    resolveQuietly RPCPeer {rpcPeerIP = pip} =
      resolveIPOrHost pip >>= \case
        Left err -> do
