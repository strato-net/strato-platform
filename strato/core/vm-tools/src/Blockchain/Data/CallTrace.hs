...
-- | Structured execution traces for the debug_* JSON-RPC endpoints.
--
-- SolidVM is a source interpreter, so the trace is a geth callTracer-style
-- call-frame tree (built from CallInfo push/pop) rather than an opcode log,
-- with optional statement-level entries carrying source positions.
--
...
-- still in the tracer.
module Blockchain.Data.CallTrace
  ( CallType (..),
    TraceLog (..),
    StatementEntry (..),
    CallFrame (..),
