...
instance HasVault IO where
  sign bs = do
    ...
    eSig <- runClientM (VC.postSignature' (T.pack "nodekey") Nothing (VC.MsgHash bs)) (mkClientEnv mgr url)
    case eSig of
...
main = do
  ...
  optSender <- do 
    eAdAndKey <- runClientM (VC.getKey (T.pack "nodekey") Nothing Nothing) (mkClientEnv mgr vaultUrl)
    case eAdAndKey of
