...
instance Format InEvent where
  ...
  format (SnapshotReceived) = "Snapshot received"  --TODO improve, ie add block number and more helpful info
...
instance Format OutEvent where
  ...
  format (OSnapshot) = "Snapshot found. Recreating state..."
...
inShortLog loc iev = $logInfoS loc . pack $
  case iev of
    ...
    SnapshotReceived -> CL.blue "SNAPSHOT_RECEIVED"
...
outShortLog loc eoev = do
  ...
  $logInfoS loc . pack $
    case fromE eoev of
      ...
      OSnapshot -> prefix ++ CL.blue "OSNAPSHOT"
