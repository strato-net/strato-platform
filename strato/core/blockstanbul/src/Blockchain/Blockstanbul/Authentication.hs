...
getX509FromAddress' address = do
  ...
  case maybeCert of
    Nothing -> throwError $ "Proposer does not have an X509 certificate: " ++ formatAddressWithoutColor address
--    Nothing -> error $ "Proposer does not have an X509 certificate: " ++ formatAddressWithoutColor address
    Just v -> return v
