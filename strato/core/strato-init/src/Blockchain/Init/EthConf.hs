...
getNodeKey = do
  ...
  ak <- waitOnVault clientEnv $ runClientM (getKey (T.pack "nodekey") Nothing Nothing) clientEnv
    ...
    Left (FailureResponse _ (Response (Status code _) _ _ body)) -> case code of
      ...
      400 -> -- 400 is thrown when the key does not exist
        if flags_generateKey then do 
          ...
          waitOnVault clientEnv $ runClientM (postKey (T.pack "nodekey" ) Nothing) clientEnv
        else do
