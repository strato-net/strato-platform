...
import Blockchain.EthConf.Model (apiConfig, httpPort, urlConfig, vaultUrl)
...
generateDockerCompose = do
  ...
  let conf = ethConf
      ...
      -- In local mode strato runs on the host, not in Docker.
      -- Docker containers must use host.docker.internal to reach it.
      stratoDockerHost = "host.docker.internal"
  ...
  let mercataBackend = def
        ...
        , environment = Just $ Map.fromList
            ...
            , ("STRATO_HOSTNAME", stratoDockerHost)
            -- TODO: move secrets to secrets file generated by strato-setup
  ...
  let postgrest = def
        ...
        , environment = Just $ Map.fromList
            [ ("blocHost", stratoDockerHost ++ ":3000")
