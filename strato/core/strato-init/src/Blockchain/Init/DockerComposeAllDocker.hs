...
generateDockerComposeAllDocker = do
  let repoUrl = flags_repoUrl
      ...
      pgPassword = "${postgres_password:-api}"  -- Docker Compose will interpolate at runtime
  ...
  let postgrest = def
        ...
        , environment = Just $ Map.fromList
            ...
            , ("PG_ENV_POSTGRES_PASSWORD", pgPassword)
  ...
  let postgres = def
        ...
        , volumes = Just ["./logs:/logs", "pgdata:/var/lib/postgresql/data"]
        , restart = Just "unless-stopped"
        , logging = noLogging
        }
