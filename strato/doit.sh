...
function doInit {
  ...
  args="--pguser=$pgUser --password=$pgPass --genesisBlockName=$genesis --kafka=./kafka-topics.sh \
        ...
        --generateKey=$generateKey --extraFaucets=$extraFaucets ${networkFlag} --genesisBlockTestCert=$genesisBlockTestCert --genesisCerts=$genesisCerts"
  ...
  if ${splitinit:-false} ; then
    ...
    init-worker --kafkahost=$kafkaHost --genesisBlockTestCert=$genesisBlockTestCert --genesisCerts=$genesisCerts 2>&1 | tee --append logs/strato-setup
    if [ ${PIPESTATUS[0]} -ne 0 ]; then
      echo "STRATO SETUP FAILED: see /var/lib/strato/logs/strato-setup for details"
      tail -f /dev/null
    fi
