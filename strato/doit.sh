...
function cleanupDB {
  ...
  PGPASSWORD=$pgPass psql ${db_conn_params} -c "
    ...
    WHERE pg_stat_activity.datname = 'strato';"
  PGPASSWORD=$pgPass psql ${db_conn_params} -c "copy (select datname from pg_database where datname = 'strato') to stdout" | while read line; do
