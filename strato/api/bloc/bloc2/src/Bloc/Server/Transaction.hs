    ...
    FUNCTION -> do
      ...
      forStateT Map.empty txsWithParams $
        \MethodCall{..} -> do
          ...
          -- See FUNCTION-case comment in postBlocTransactionResult: the
          -- cache must hold the CodeCollection too, otherwise repeat
          -- methodcalls in the same batch lose file-level structs.
