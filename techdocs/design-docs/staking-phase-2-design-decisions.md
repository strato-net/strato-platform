...
| D10 | **Eligibility = `selfBond ≥ minStake` (10k), not jailed, no exit due** (revised 2026-09-14; originally `selfBond + delegated ≥ minStake` with a separate `minSelfBond` floor). `selfBondGraceUntil` keeps the delegated rule for validators admitted under it until an admin-chosen deadline. | A validator with nothing of its own at stake should not validate purely on delegations; the grace deadline lets helium's delegation-only validators bond without being removed on upgrade. | Self-bond + delegated (the original rule). |
