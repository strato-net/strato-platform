...
describe('Stripe Endpoint Tests', function() {
  ...
  it('Onboard without accountId', async () => {
    const res = await request(app).get('/Stripe/onboard').set('referer', marketplaceUrl);
    expect(res.statusCode).toBe(200);
  });

  it('Onboard with accountId', async () => {
    const res = await request(app).get(`/Stripe/onboard/${testAccountId}`).set('referer', marketplaceUrl);
    expect(res.statusCode).toBe(200);
  });

  it('Check the status of a Stripe account', async () => {
    const res = await request(app).get(`/Stripe/status/${testAccountId}`);
    expect(res.statusCode).toBe(200);
  });
  ...
  if (testSellerId && testSessionId) {
    ...
    it('Retrieve the Stripe session given sessionId', async () => {
      const res = await request(app).get(`/Stripe/session/${testSessionId}/${testSellerId}`);
      expect(res.statusCode).toBe(200);
    });

    it('Retrieve the Stripe intent given sessionId', async () => {
      const res = await request(app).get(`/Stripe/intent/${testSessionId}/${testSellerId}`);
      expect(res.statusCode).toBe(200);
    });
  }
  else {
