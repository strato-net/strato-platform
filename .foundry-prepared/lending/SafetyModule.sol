...
import "../compat/TokenFactory.sol";
...
contract SafetyModule is Ownable {
    ...
    function totalShares() public view returns (uint) { return IERC20(sToken).balanceOf(address(this)); }
    ...
    function stake(uint assetsIn, uint minSharesOut) external onlyActiveToken(asset) onlyActiveToken(sToken) returns (uint sharesOut) {
        ...
        // emulate share mint by accounting (keep simple: track shares on SM balance)
        // transfer shares in by increasing SM's sToken balance is out of scope; we keep balance-based accounting
    ...
    function startCooldown() external {
        require(totalShares() > 0, "SM:no shares");
    ...
    function redeem(uint sharesIn, uint minAssetsOut, address to) external returns (uint assetsOut) {
        require(sharesIn > 0, "SM:zero");
        require(totalShares() >= sharesIn, "SM:not holder");
        ...
        // burn then transfer
        // burn simulated by reducing internal shares; omitted for brevity in this minimal wiring
