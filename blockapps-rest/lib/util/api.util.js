...
async function postWithFullResponse(url, endpoint, _body, options) {
  function createBody(_body, options) {
    // array
    if (Array.isArray(_body)) return _body;
    // object
  ...
  const body = createBody(_body, options);
  return ax.postWithFullResponse(url, endpoint, body, options);
}
...
async function getWithFullResponse(host, endpoint, options = {}) {
  return ax.getWithFullResponse(host, endpoint, options);
}
...
async function postueWithFullResponse(host, endpoint, data, options) {
  // TODO - @samrit do we need txParams here
  return ax.postueWithFullResponse(host, endpoint, data, options);
}
