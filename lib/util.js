...
const typeMapping = {'Bytes':'text', 'Bool':'boolean', 'String':'text', 'Int':'numeric(80,0) DEFAULT 0', 'Address':'text', 'json':'json DEFAULT \'{}\''}
...
var toSchemaString = function(json){
  ...
  var dropTable = `DROP TABLE IF EXISTS "${json.name}"; `;
  var tableCreate = `CREATE TABLE IF NOT EXISTS "${json.name}" (${end.join(', ')}); `;
  var indexCreate = `CREATE INDEX IF NOT EXISTS idx ON "${json.name}" (address); `;
  var deleteFromContracts = `DELETE FROM "contract" WHERE name='${json.name}'; `
  var nameAdd = `INSERT INTO contract VALUES (DEFAULT, '${json.codeHash}', '${json.name}', '${JSON.stringify(json)}' ) ON CONFLICT DO NOTHING; `;

  return "BEGIN; " + dropTable + tableCreate + indexCreate + nameAdd + " COMMIT;"
}
