...
const DETERMINISTIC_ROUTE_ERROR =
  /cap exceeded|eab: (invalid route|route discontinuity|route output mismatch|route source mismatch|zero route)|invalid action|invalid .*factory|invalid .*pair|invalid .*pool|no executable route|no route found|no step output|paused|route quote does not satisfy|slippage|tr:|unapproved .*vault|unsupported/i;
