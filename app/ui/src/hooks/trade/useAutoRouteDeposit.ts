...
export function useAutoRouteDeposit() {
  ...
  const execute = async ({
    ...
    if (!network.depositRouter) throw new Error("Deposit router is unavailable");
    ...
    try {
      ...
      const validation = await validateRouterContract({
        ...
        tokenAddress: isNative ? NATIVE_TOKEN_ADDRESS : route.externalToken,
        targetStratoToken: route.stratoToken,
      });
      if (!validation.isValid) {
        throw new Error(validation.error || "Bridge deposit validation failed");
      }
