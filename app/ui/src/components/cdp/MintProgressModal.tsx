...
const MintProgressModal: React.FC<MintProgressModalProps> = ({
  ...
  const getStatusBadge = (tx: TransactionProgress) => {
    if (tx.status === "completed") {
      return <span className="text-xs px-2 py-0.5 rounded-full bg-success/15 text-success font-medium">Completed</span>;
    }
    if (tx.status === "processing") {
      return <span className="text-xs px-2 py-0.5 rounded-full bg-primary/15 text-primary font-medium">In progress</span>;
    }
    if (isSkipped(tx)) {
      return <span className="text-xs px-2 py-0.5 rounded-full bg-gray-500/20 text-gray-400 font-medium">Cancelled</span>;
    }
    if (tx.status === "error") {
      return <span className="text-xs px-2 py-0.5 rounded-full bg-destructive/15 text-destructive font-medium">Failed</span>;
    }
          ...
          {transactions.map((tx, index) => {
            ...
            return (
              ...
              >
                {isCollapsed ? (
                  <div 
                    className="flex items-center gap-3 px-4 py-2 transition-colors cursor-pointer hover:bg-success/15"
                  ...
                  >
                    ...
                    <span className="text-xs text-success">Click to expand</span>
                  </div>
                ) : (
