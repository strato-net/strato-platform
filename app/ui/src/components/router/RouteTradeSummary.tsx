...
export default function RouteTradeSummary({ quote, inputAmount, inputDecimals, inputSymbol, outputToken, external, fetching, error }: {
  ...
  return <div className="space-y-2 text-xs">
    <p className={`min-h-5 flex items-start gap-2 ${error ? "font-medium text-destructive" : "text-muted-foreground"}`} role={error ? "alert" : "status"}>
      {error && <AlertCircle className="h-4 w-4 shrink-0" aria-hidden="true" />}
      <span>{error || (fetching ? quote ? "Updating quote…" : "Getting quote…" : quote ? "Quote ready" : "Enter an amount to get a quote")}</span>
    </p>
