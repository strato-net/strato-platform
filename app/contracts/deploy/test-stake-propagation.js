...
// MercataGovernance. Stakes a small amount on one validator and reports
// governance's validatorStake before and after.
//
//   node test-stake-propagation.js [validator] [amountWei]
...
const VALIDATOR = (process.argv[2] || '0c4cecae296c33f71f9a6e6fb57f418f9d5f7e82').replace(/^0x/, '');
...
(async () => {
  ...
  const before = await govStake(VALIDATOR);
  ...
  const resp = await rest.call(tokenObj, {
    ...
    method: 'stake',
    args: { validator: { type: 'address', value: VALIDATOR }, amount: AMOUNT },
    txParams: { gasPrice: config.gasPrice, gasLimit: config.gasLimit },
  }, { config, isAsync: true, cacheNonce: true });
  ...
  const after = await govStake(VALIDATOR);
