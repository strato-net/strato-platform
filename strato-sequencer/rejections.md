...
+ `T` is put into `/transaction` with `T.blocknumber = -1`
 ...
 + If `nonce(T.from) + 1 < T.nonce < `nonce(T.from) + L` where `L` is the window parameter of `strato-sequencer`, it will be kept in the queue and processed once `nonce(T.from)` reaches `T.nonce`.
 + If `T'` and `T` enter the queue with `T.nonce = T'.nonce` then the transaction with the lowest `gasLimit*gasPrice` will be dropped. @ilya Can we set a new "dropped message"?
